  ...
  <view>
    ...
    <view class="app-card">
      <view class="flex-between cell">
        <view class="cell-label">{{ info.createTime }}</view>
        ...
        <view v-show="info.rpShow">
          ...
          <view v-for="rp in info.rpList" :key="rp.rpNo">
            <view class="prescription-item">
              ...
              <view class="">
                <view class=""
                  >审方医生：{{ rp.auditName }} {{ rp.auditTitle }}</view
                >
              ...
              <view class="flex-between">
                ...
                <!-- <view @click="rp.drugShow = !rp.drugShow">
                  {{ rp.drugShow ? '收起处方' : '展开处方' }}
                  <image
                    class="more__icon"
                    :class="{ reverse: rp.drugShow }"
                    :src="require('@/assets/zhankaihui@2x.png')"
                    mode="aspectFit"
                  ></image>
                </view> -->
              ...
              <view
                class="drug-item"
                v-for="drug in rp.contentList"
                :key="drug.id"
              >
                <view class="flex-between">
                  <view class=""
                    >{{ drug.name }}(规格：{{ drug.spec }}) x{{
                      drug.total
                    }}</view
                  >
                  <view class=""
                    >医保：{{ swithMedicare(drug.medicare) || '-' }}</view
                  >
                </view>
                <view class="color-orange"
                  >{{ drug.useWays }} {{ drug.useFrequency }}，{{
                    drug.singleDose
                  }}{{ drug.singleDoseUnitText }}/次</view
                >
      ...
      <view class="cell">
        <view class="flex-between">
          ...
          <view @click="info.examInfoShow = !info.examInfoShow">
            ...
            <image
              class="more__icon"
              :class="{ reverse: info.examInfoShow }"
              :src="require('@/assets/zhankaihui@2x.png')"
              mode="aspectFit"
            ></image>
          </view>
        </view>
        <view v-show="info.examInfoShow">
          <view
            class="drug-item"
            v-for="exam in info.examInfoList"
            :key="exam.reportNo"
          >
            <view class="flex-between">
            ...
            <view style="font-size: 32rpx;">{{
              exam.clinicDiag || exam.reportTitle
            }}</view>
            <view class="flex-between">
      ...
      <view class="cell">
        <view class="flex-between">
          ...
          <view @click="info.labInfoShow = !info.labInfoShow">
            ...
            <image
              class="more__icon"
              :class="{ reverse: info.labInfoShow }"
              :src="require('@/assets/zhankaihui@2x.png')"
              mode="aspectFit"
            ></image>
          </view>
        </view>
        <view v-show="info.labInfoShow">
          <view
            class="drug-item"
            v-for="exam in info.labInfoList"
            :key="exam.reportNo"
          >
            <view class="flex-between">
              <view class="">报告编号：{{ exam.reportNo }}</view>
            ...
            <view class="flex-between">
              <view class="">报告医生：{{ exam.reportDocName }}</view>
              <view class="">报告时间：{{ exam.reportDtime || '-' }}</view>
            </view>
          </view>
        </view>
      ...
      <view class="cell">
        <view class="flex-between">
          ...
          <view @click="info.medicalRecordShow = !info.medicalRecordShow">
            ...
            <image
              class="more__icon"
              :class="{ reverse: info.medicalRecordShow }"
              :src="require('@/assets/zhankaihui@2x.png')"
              mode="aspectFit"
            ></image>
          </view>
        </view>
        <view v-show="info.medicalRecordShow">
          <view>
            <text class="">病历编号</text>
            <text class="ml-50">{{
              info.medicalRecord.visitNo || info.medicalRecord.medicalNo
            }}</text>
          </view>
          <view>
            <text class="">就诊人</text>
          ...
          <view style="color: #666;">{{info.medicalRecord.advice}}</view> -->
            <!-- 病历更改后 -->
            <view class="mt-20" :key="item">就诊方式：</view>
            <view style="color: #666;">{{ info.medicalRecord.seeWay }}</view>
            <view class="mt-20" :key="item">病情：</view>
            <view style="color: #666;">{{ info.medicalRecord.illness }}</view>
            <view class="mt-20" :key="item">辅助检查：</view>
            <view style="color: #666;">{{ info.medicalRecord.supCheck }}</view>
            <view class="mt-20" :key="item">初次诊断：</view>
            <view style="color: #666;">{{ info.medicalRecord.diagDesc }}</view>
            <view class="mt-20" :key="item">处理意见：</view>
            <view style="color: #666;">{{ info.medicalRecord.advice }}</view>
          </template>
