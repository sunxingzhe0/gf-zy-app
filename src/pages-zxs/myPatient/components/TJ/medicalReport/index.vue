  ...
  <view class="medical-report">
    ...
    <template v-for="(one, menu) in menuList">
      ...
      <template v-if="one.tagType == 'LCXJ'">
        <collapse-item
          :title="'【体检】' + one.tagName"
          :nums="tableList.length"
          :key="menu"
        >
          <view
            class="table-box"
            v-for="(item, table) in one.beanList"
            :key="table"
          >
            <view class="title">{{ item.title }}</view>
            <table cellspacing="0">
              <thead>
                <tr>
                  <td v-for="(th, thead) in tableHead" :key="thead">
                    {{ th }}
                  </td>
                </tr>
              </thead>
            ...
            <view class="remark"
              >小结内容：小结内容小结内容小结内容小结内容小结内
              容小结内容小结内容小结内容小结内容</view
            >
          </view>
        </collapse-item>
      </template>
      ...
      <template v-if="one.tagType == 'JCXJ'">
        <collapse-item :title="'【体检】' + one.tagName" :key="menu">
          <view
            class="item-one"
            v-for="(one, oneBox) in one.beanList"
            :key="oneBox"
          >
...
export default {
  ...
  data() {
    return {
      menuList: [
        {
          ...
          beanList: [
            ...
            {
              title: "体检综述",
              text: "1.双下肺感染性病变;度影，增强后有强化,余肺纹理清晰。两肺门区未2.腹水;见异常。气管支气管影通畅，未见明显狭窄。纵隔",
            },
          ],
        },
      ],
      ...
      oneList: [
        { title: "体检报告时间", text: "2020-06-05" },
        {
          title: "体检综述",
          text: "这里是体检综述这里是体检综述这里是体检综 述这里是体检综述",
        },
      ],
      twoList: [
        { title: "体检症状", text: "牙周炎，超重" },
        {
          title: "建议内容",
          text: "这里是建议内容这里是建议内容这里是建议内 容这里是建议内容",
        },
      ],
      tableList: [
        {
      ...
      summaryList: [
        {
          childen: [
            ...
            {
              title: "检查所见",
              text: "三维重建轴位、冠状位及矢状位图像示:临床提 示:后纵隔病变;右肺中叶见条片状高密度影",
            },
            {
              title: "检查结论",
              text: "1.双下肺感染性病变; 度影，增强后有强化,余肺纹理清晰。两肺门区 未 2.腹水; 见异常。气管支气管影通畅，未见明显狭窄。 纵隔 3.胆囊炎并结石; 请结合临床建议必要时进一步检查",
            },
            { title: "小结内容", text: "" },
          ],
        },
