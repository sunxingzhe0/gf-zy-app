    ...
    <view v-for="(item, index) in messageBox" :key="index">
      <view v-if="self(item)" class="flex-end-start self chat-item">
        <view class="content-wrapper">
          <view class="chat-content" style="font-size:14px">{{
            item.body
          }}</view>
        </view>
      ...
      <view v-else class="flex-start-start left chat-item">
        ...
        <view class="chat-content"
          >{{ item.isProblem ? item.body : item.problems }}
          <view class="tips" v-if="item.isProblem">
            <view
              @click="sendClickMsg(val)"
              v-for="(val, i) in item.problems"
              :key="i"
              style="color:#0AB2C1;font-size:14px"
              >{{ `${i + 1}. ${val}` }}</view
            >
          </view>
        </view>
