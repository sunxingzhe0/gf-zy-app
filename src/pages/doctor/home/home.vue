<template>
  <view class="wrapper">
    <view class="homeTop">
      ...
      <text>{{ getTimeState }}好 {{ userInfo.username }} 医生，欢迎登录{{ userInfo.orgName }}。</text>
    </view>
    <view class="homeWrap">
      <view class="box-shadow listItem flex-between navWrap">
        <view class="navItem" @click="navigate('/pages-doctor/room/room?navTitle=CONSULT')">
          <image class="icon" :src="require('@/assets/user/navIcon1.png')"></image>
          <view class="title">在线咨询</view>
          <text v-if="messages.CONSULT">{{ messages.CONSULT }}</text>
        </view>
        <view class="navItem" @click="navigate('/pages-doctor/room/room?navTitle=REPEAT_CLINIC')">
          <image class="icon" :src="require('@/assets/user/navIcon3.png')"></image>
          <view class="title">在线复诊</view>
          <text v-if="messages.REPEAT_CLINIC">{{ messages.REPEAT_CLINIC }}</text>
        </view>
        <view class="navItem" @click="navigate('/pages-doctor/room/room?navTitle=CARRYON_PRESC')">
          <image class="icon" :src="require('@/assets/user/navIcon2.png')"></image>
          <view class="title">慢病续方</view>
          <text v-if="messages.CARRYON_PRESC">{{ messages.CARRYON_PRESC }}</text>
        </view>
      </view>
      <view class="box-shadow listItem flex-between notice" v-if="notice"
        @click="toNotice">
        <image :src="require('@/assets/user/notice.png')"></image>
        <view class="text-overflow noticeText">
          <uni-notice-bar background-color="#fff" style="padding: 0;margin-bottom:0;"
            color="#1A1A1A" scrollable="true" :speed="50" single="true" :text="notice"></uni-notice-bar>
        </view>
        ...
        <view class="date">
          <view class="dateTop flex-start-center">
            <view class="flex-start-center flex_1 p030">
              ...
              <view class="flex-start-center" style="border-left: 1px solid #CCCCCC;margin-left: 30rpx;padding-left: 30rpx;">
            ...
            <view @click="dateOpen" class="p030">
              <image :src="isdate ? require('@/assets/hide.png') : require('@/assets/show.png')"></image>
            </view>
          </view>
          <view v-show="isdate">
            <uni-calendar :insert="true" :showMonth="false" ref="date" @change="change" />
          </view>
          <view class="weekWrap">
            <scroll-view class="scroll-view_H" scroll-x="true" scroll-left="0">
              <view :class="['weekItem', index == isweek ? 'on' : '']" v-for="(item, index) in weeks"
                :key="index" @click="changeWeek(index)">
                <view>{{ item.week }}</view>
          ...
          <view class="plan_main">
            <view style="font-size: 26rpx; text-align: center;color: #999;"
              v-if="!weeks[isweek].workDay">{{ weeks[isweek].workDay ? '工作日' : '休息日' }}</view>
            <view class="flex-between planShow" v-show="!isPlan && weeks[isweek].workDay"
              style="margin-bottom: 20rpx;">
              <view class="total">{{ weeks[isweek].hasOrder }}</view>
              <view class="names">
                <view class="patient" v-for="(f, i) in weeks[isweek].nameList"
                  :key="i">{{ f }}</view>
              </view>
              <view class="left">
                剩余
            ...
            <view v-show="isPlan">
              <block v-for="(item, index) in weeks[isweek].workTime" :key="index">
                <view class="flex-between planShow paddt" :key="index" v-if="item.freeType != '空闲'">
                  <view class="flex-start-center planItem">
                    <view class="planTime">
                      <view>{{ item.timeRange.split('-')[0] }}</view>
                      <view>{{ item.timeRange.split('-')[1] }}</view>
                      <image class="ljIcon" :src="require('@/assets/timeRange.png')"></image>
                    </view>
                    <view class="flex-start-center">
                      <view class="patient" v-for="(f, i) in item.userName"
                        :key="i">{{ f }}</view>
                    </view>
                  </view>
                  <view class="planNum">
            ...
            <view class="openIcon" v-if="weeks[isweek].hasOrder>0" @click="isPlan = !isPlan">
              <image :src="isPlan ? require('@/assets/hide.png') : require('@/assets/show.png')"></image>
            </view>
          </view>
        </view>
      </view>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      uni.$on('onMessage', this.getSessionMessage);
    },
    onReady() {
    ...
    methods: {
      ...
      preYear() {
        this.$refs.date.setDate(dayjs(this.calendar.fullDate).subtract(1,
          'year'));
        this.calendar = this.$refs.date.nowDate;
      },
      nextYear() {
        ...
        if (this.$store.state.userInfo.clientType == 'DOC_MOBILE') {
          ...
          TYPES.forEach(type =>
            Object.assign(MESSAGES, {
              [type]: sessionList[type]?.map(({
                sessionId
              }) => sessionId).reduce((num, id) => num + this.webSocket
                .getUnread(id)?.length, 0)
            })
          );
