<template>
  <view class="clinic-diagnosisList" :style="ispop?'padding-left:0;padding-right:0':''">
    <scroll-view class="departments" v-if="ispop" scroll-y style="height:70vh;background-color: #f2f2f2;">
      <view style="margin:0 20rpx;">
        <app-card styles="padding: 0;" class="listWrap" v-for="(item,index) in tableData" :key="index">
          <view class="listItem">
            <app-list-item :label="`诊断${index+1}`" :styles="{paddingLeft:0}" textStyle="font-size:30rpx;color:#333">
              <view class="list_right">
                设为主诊断
                <switch :disabled="item.mainDiagnosis == 1 ? true : false" :checked="item.mainDiagnosis == 1 ? true : false"
                  @change="changeMainDiagnosis($event,item,index)" color="#0AB2C1" />
              </view>
            </app-list-item>
            <app-list-item label="类别" :styles="{paddingLeft:0,paddingBottom:item.typeOpen?0:''}" textStyle="font-size:26rpx;color:#333">
              <view class="list_right" @click="typeTop(item)">
                {{getTypeName(item.diagnosisType)}}
                <uni-icons :type="item.typeOpen?'arrowdown':'arrowright'" size="14" style="margin-left: 20rpx;" color="#666"></uni-icons>

              </view>
            </app-list-item>
            <view class="types" v-if="item.typeOpen">
              <view :class="item.diagnosisType == type.value?'typesItem on':'typesItem'" v-for="type in types" @click="changeType(item,type.value)"
                :key="type.value">{{type.label}}</view>
            </view>
            <app-list-item label="诊断/病名" :styles="{height:auto,alignItems: 'flex-start',padding:'20rpx 20rpx 10rpx 0rpx'}"
              textStyle="font-size:26rpx;color:#333;line-height: 80rpx;">
              <view class="list_right" style="text-align: left;">
                <view class="inputText">
                  <input class="uni-input" v-model="item.prefixName" placeholder="前缀" />
                </view>
                <view class="inputText" @click="selectDiaName(index+1)">
                  <input class="uni-input" v-model="item.diagnosisName" disabled placeholder="请输入" />
                </view>
                <view class="inputText" style="border: 0;">
                  <input class="uni-input" v-model="item.suffixName" placeholder="后缀" />
                </view>
              </view>
            </app-list-item>
            <view class="listBtn flex-justify-end">
              ...
              <view v-if="index+1<tableData.length && index+1!=tableData.length" @click="changeSort('down', index)">下移</view>
              <view v-if="index<tableData.length && index!=0" @click="changeSort('up', index)">上移</view>
              <picker class="item-picker" mode="multiSelector" range-key="name" @change="classifyChange($event,index)"
                @columnchange="columnchange" :value="classifyIndex" :range="classifyArr">
                <view v-if="tableData.length>1 && item.mainDiagnosis!=1 && item.childDtos.length==0">移动至</view>
              </picker>

            </view>
          </view>
          <view class="listItem" v-for="(child,d) in item.childDtos" :key="d">
            <app-list-item :label="`诊断${index+1}-${d+1}`" :styles="{paddingLeft:0}" textStyle="font-size:30rpx;color:#333">
              <view class="list_right">
                设为主诊断
                <switch :disabled="child.mainDiagnosis == 1 ? true : false" :checked="child.mainDiagnosis == 1 ? true : false"
                  @change="changeMainDiagnosis($event,child,index,d)" color="#0AB2C1" />
              </view>
            </app-list-item>
            <app-list-item label="诊断/病名" :styles="{height:auto,alignItems: 'flex-start',padding:'20rpx 20rpx 10rpx 0rpx'}"
              textStyle="font-size:26rpx;color:#333;line-height: 80rpx;">
              <view class="list_right" style="text-align: left;">
                <view class="inputText">
                  <input class="uni-input" v-model="child.prefixName" placeholder="前缀" />
                </view>
                <view class="inputText" @click="selectDiaName(index+1,d+1)">
                  <input class="uni-input" v-model="child.diagnosisName" disabled placeholder="请输入" />
                </view>
                <view class="inputText" style="border: 0;">
                  <input class="uni-input" v-model="child.suffixName" placeholder="后缀" />
                </view>
              </view>
            </app-list-item>
            <view class="listBtn flex-justify-end">
              ...
              <view v-if="d+1<item.childDtos.length && d+1!=item.childDtos.length" @click="changeSort('down',index, d )">下移</view>
              <view v-if="d<item.childDtos.length && d!=0" @click="changeSort('up',index, d)">上移</view>
            </view>
          </view>
        </app-card>
      </view>
    </scroll-view>
    <app-card v-else styles="padding: 0;" class="listWrap" v-for="(item,index) in tableData" :key="index">
      <view class="listItem">
        <app-list-item :label="`诊断${index+1}`" :styles="{paddingLeft:0}" textStyle="font-size:30rpx;color:#333">
          <view class="list_right">
            设为主诊断
    ...
    <historyTem ref="historyTem" type="1" @submit="historyImportSubmit" :memberId="memberId" title="历史诊断模板"></historyTem>
    <historyTem ref="commonTem" type="2" @submit="historyImportSubmit" :memberId="memberId" title="常用诊断模板"></historyTem>
    <!--选择移动至-->
      ...
      <view class="select_wrap">
        ...
        <view class="select_content">
          <picker-view :value="value" @change="bindChange">
            <picker-view-column>
              <view class="item" v-for="(item,index) in tableData" :key="index">诊断{{index+1}}</view>
            </picker-view-column>
            <picker-view-column>
              <view class="item" v-for="(item,index) in 6" :key="index">{{index}}</view>
            </picker-view-column>
          </picker-view>
        </view>
      </view>
    ...
    methods: {
      ...
      classifyChange(e, index) {
        ...
        if (value[0] == index) {
          ...
          this.tableData[value[0]].childDtos.splice(value[1], 0, this.tableData[index])
      ...
      onInspectNameChange(val) {
        if (this.selectIndex.length == 1) {
          this.tableData[this.selectIndex[0] - 1].diagnosisName = val.diagnosisName
        ...
        if (this.selectIndex.length == 2) {
          this.tableData[this.selectIndex[0] - 1].childDtos[this.selectIndex[1] - 1].diagnosisName = val.diagnosisName
          this.tableData[this.selectIndex[0] - 1].childDtos[this.selectIndex[1] - 1].diagCode = val.diagnosisCode
          this.tableData[this.selectIndex[0] - 1].childDtos[this.selectIndex[1] - 1].diagTypeCode = val.diagnosisType
        }
      ...
      add() {
        const data = {
          ...
          diagTypeCode: '',
          suffixName: '',
          indexCode: this.tableData.length == 0 ?
            1 : this.getindexcode() + 1,
      ...
      changeSort(type, index, d) {
        ...
        if (type == 'up') {
          if (d) {
            this.swapArray(this.tableData[index].childDtos, childIndex - 1 - 1, childIndex - 1);
          } else {
        ...
        if (type == 'down') {
          if (childIndex) {
            this.swapArray(this.tableData[index].childDtos, childIndex - 1, childIndex);
          } else {
      ...
      addChild(
        ...
        this.tableData[$index].childDtos = [...this.tableData[$index].childDtos, childDtos]
        this.getAllClassify()
      },
      confirm() {
        const params = {
