<template>
  <view>
    ...
    <view class="orderTop flex-between">
      ...
      <text class="sate">{{ infoData.serviceType == 'CONSULT' ? '在线咨询' : infoData.serviceType == 'REPEAT_CLINIC' ? '在线复诊' : '慢病续方' }}</text>
    </view>
    <view class="orderWrap " :style="{paddingBottom:infoData.status == 'PENDING_REVIEW' || infoData.status == 'REJECTED' || infoData.status == 'DRAFT'?'':'40rpx'}">
      <view class="oerderMain paddtb box-shadow">
        ...
        <view class="orderItem border0 flex-between">
          ...
          <view class="info">
            <view>{{ infoData.serviceType == 'CONSULT' ? '在线咨询' : infoData.serviceType == 'REPEAT_CLINIC' ? '在线复诊' : '慢病续方' }}</view>
          </view>
          <uni-icons type="arrowright" class="rightIcon hide"></uni-icons>
        </view>
        <view class="orderItem border0 flex-between">
      ...
      <view class="oerderMain paddtb box-shadow">
        ...
        <view class="orderItem border0 flex-start-start">
          <view :class="['info', isOpen ? '' : 'text-overflow']">{{ infoData.diagnosis }}</view>
          <view class="font24 color999" @click="isOpen = !isOpen">
            {{ isOpen ? '收起' : '展开' }}
            <text :class="['iconfont', isOpen ? 'icon-expand-down' : 'icon-zhankai', 'font24', 'color999']"></text>
          </view>
        </view>
      </view>
      <view class="oerderMain paddtb box-shadow">
        ...
        <view>
          <view class="orderItem border0">
            <view class="flex-between">
              <view class="color333 font26">西药:￥{{ infoData.westPrice }}</view>
              <view class="color333 font26">中药:￥{{ infoData.chPatentPrice }}</view>
              <view class="pricolor font26">合计:￥{{ infoData.prescriptionPrice }}</view>
            </view>
            <view class="recipelInfo" v-for="(item, index) in infoData.rpDrugList" :key="index">
              <view class="recipelItem" style="margin-bottom: 0;">
                <view class="flex-between">
                  <view class="font26">{{ index + 1 }}.{{ item.name }}（规格：{{ item.spec }}）</view>
                  <view class="font26 color666">{{ item.medicare == 'ME_FEE' ? '自费' : swithMedicare(item.medicare) }}</view>
                </view>
                <view class="colorred font26 paddingLeft16 flex-start-start">
                  {{ allchannel[item.useWays] || '' }} {{ allfrequency[item.useFrequency] || '' }} {{ item.singleDose || '' }}{{ item.singleDoseUnitText + '/次' }} {{ item.treatment || ''
                  }}{{ attrdate[item.treatmentUnit] || '' }}
                  <view class="color666 ml-20">￥{{ item.price }}{{ '/' + item.alltotalUnit[item.totalUnit] }}x{{ item.total }}</view>
                </view>
                <view class="cloum">
      ...
      <view class="oerderMain  box-shadow paddtb">
        <view class="orderItem paddtb">
          <view class="flex-between">
            ...
            <view class="info" style="word-wrap: break-word;">{{ infoData.prescriptionNo || infoData.id }}</view>

            <view class="pricolor" @click="cope(infoData.prescriptionNo || infoData.id)">复制</view>
          </view>
          <view class="flex-between">
        ...
        <view class="orderItem  flex-between ">
          ...
          <view class="info flex-start-center">
            <default-img class="headImg" :url="FILE_URL(infoData.doctorAvatar)"></default-img>
            <view>
              {{ infoData.doctorName }}
        ...
        <view class="flex-between orderItem  border0" v-show="infoData.status != 'PENDING_REVIEW' && infoData.status != 'DRAFT'">
        ...
        <view class=" flex-between orderItem " v-show="infoData.status != 'PENDING_REVIEW' && infoData.status != 'DRAFT'">
          <view class="title">审方医生</view>
          <view class="info flex-start-center">
            <default-img class="headImg" :url="FILE_URL(infoData.auditAvatar)"></default-img>
            <view>
              {{ infoData.auditUserName }}
              <text class="color666 paddingLeft16">{{ infoData.auditUserTitle }}</text>
            </view>
          </view>
        </view>

        <view class="flex-between orderItem border0" v-show="infoData.status == 'REJECTED'">
      ...
      <view v-for="(item, index) in infoData.relRpIds" class="oerderMain   paddtb" :key="index">
        <view class="orderItem border0 flex-between">
          <view class="title">关联处方</view>
          <view class="info" style="word-wrap: break-word;">{{ item }}</view>
          <text class="pricolor otherCope font26" @click="copyOther(item)">复制</text>
          <text class="pricolor font26" @click="seeRelation(item)">查看</text>
        </view>
      </view>
    </view>
    <view class="fiexBtn flex-justify-end" style="text-align: right;" v-if="infoData.status == 'PENDING_REVIEW' || infoData.status == 'REJECTED' || infoData.status == 'DRAFT'">
      <view class="chfBtn on" v-if="infoData.status == 'PENDING_REVIEW'" @click="withdraw">撤回</view>
      <!-- 驳回 状态时 点击复制相当于新增一个处方-->
      <view class="chfBtn on" v-if="infoData.status == 'REJECTED'" @click="copy">复制</view>
      <view class="chfBtn on" v-if="infoData.status == 'DRAFT'" @click="del">删除</view>
      <view class="chfBtn on" v-if="infoData.status == 'DRAFT'" @click="submit(infoData)">提交</view>
    </view>

    <!-- 模态框 -->
    <dialog-pop v-if="isShowSetModel" @close='hideSetModel' :hideFooter="true" >
      <DrugsorList :codeId="true" :prescriptionNumber="prescriptionNumber" :deptId="deptId" :edittorFormDatas="edittorFormDatas" :prescriptionListData="prescriptionListData" :newPharmacy="newPharmacy" ref="getPrams"/>
    </dialog-pop>
  </view>
</template>
...
import { saveRp, delRp, withdrawRp, submitRp, orderList } from '../../../common/request/index.js';
  ...
  methods: {
    ...
    async getsingleRp() {
      ...
      this.infoData.rpDrugList.forEach((item)=>{
        ...
        item.drugTypeCode==="WESTERN_MEDICINE"?(this.infoData.westPrice+=(+item.totalPrice)):(this.infoData.chPatentPrice+=(+item.totalPrice))
        
        ////总量单位枚举
        if(item.split){
          item.alltotalUnit={[item.regularUnit]:item.regularUnitText,[item.basicUnit]:item.basicUnitText}
    ...
    async copy() {
      ...
      let resp = await addAndEdit({id:this.newCode,rpDrugList:this.infoData.rpDrugList})
      if(resp){
        uni.showToast({
          title: '复制成功！',
          icon: 'none',
      ...
      if(data==='保存'){
       ...
       /*  console.log( this.infoData)
        ...
        lists.rpDrugList.forEach((item,index)=>{
          item.id===this.editId&&(lists.rpDrugList[index]=this.params.drugMsg)
        })
        console.log(lists)
        //保存编辑后的药品
