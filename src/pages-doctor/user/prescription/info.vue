<template>
  <view>
    ...
    <view class="orderWrap ">
      ...
      <view
        class="oerderMain paddtb box-shadow"
        v-for="(item, index) in dataList"
        :key="item.id"
      >
        <view class="orderItem border0 flex-between">
          <view
            class="font-weight color333"
            @click="info('/pages-doctor/user/prescription/info')"
            >处方号：{{ item.id }}</view
          >
          <view class="type">
            {{
          ...
          <view class="orderItem border0">
            <view class="flex-between">
              <view class="color666 font20">
                医生：
                <text class="colora1a1"
                  >{{ item.doctorName || '' }} {{ item.title || '' }}</text
                >
              </view>
              <view class="color666 font20">
                创建时间：
            ...
            <view class="flex-between">
              ...
              <view class="color333 font26"
                >中成药:￥{{ item.chPatentPrice }}</view
              >
              <view class="colorred font26"
                >合计:￥{{ item.prescriptionPrice }}</view
              >
            </view>
            <view class="flex-between">
              <view class="flex-start-center">
                ...
                <text class="font24 color333"
                  >数量：{{ item.rpDrugList.length }}</text
                >
              </view>
              <view
                class="font24 color999"
                @click="dataList[index].rpShow = !dataList[index].rpShow"
              >
                {{ item.rpShow ? '收起' : '展开' }}
                <text
                  :class="[
                    ...
                    'font24',
                    'color999',
                    item.rpShow ? 'icon-expand-down' : 'icon-zhankai',
                  ]"
                ></text>
              </view>
            </view>
            <view
              class="recipelInfo"
              v-show="item.rpShow"
              v-for="(contentItem, contentIndex) in item.rpDrugList"
              :key="contentItem.id"
            >
              <view class="recipelItem" style="margin-bottom: 0;">
                <view class="flex-between">
                  ...
                  <view class="font26 color666">{{
                    swithMedicare(contentItem.medicare)
                  }}</view>
                </view>
                <view class="colorred font26 paddingLeft16"
                ...
                >
                <view class="price paddingLeft16"
                  >￥{{ contentItem.totalPrice }}</view
                >
                <view class="font24 color666 paddingLeft16"
                  >备注：{{ contentItem.remark || '-' }}</view
                >
                <view
                  class="flex-between paddingLeft16"
                ...
                >
                  ...
                  <view class="flex-start-center opt">
                    <view class="font24 color666">
                      <text
                        class="iconfont icon-shanchu font24 color666"
                      ></text>
                      删除
                    </view>
                    <view
                      class="font24 color666"
    ...
    <uni-popup ref="popup" type="center">
      <view class="selctTemplet box-shadow">
        ...
        >
          <picker-view-column>
            <view class="item" v-for="item in templetList" :key="item.id">{{
              item.lable
            }}</view>
          </picker-view-column>
        </picker-view>
...
import { findDiagnosisInClinic } from '@/common/request/prescriptionList'
...
export default {
  ...
  data() {
    return {
      ...
      indicatorStyle:
        'border:1px  solid #FFE6E6E6;border-left:0;border-right:0;color:#FF1A1A1A;',
      isShowModel: false,
      //痕迹查看弹窗数据
      modelData: {
        modalTitle: '处方提交记录',
  ...
  methods: {
    getDiagnosis() {
      findDiagnosisInClinic({ orderId: this.orderId }).then(data => {
        this.diagnosis = data
          .map(
            c =>
