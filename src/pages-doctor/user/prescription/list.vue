<template>
  <view class="orderWrap2">
    ...
    <view :class="['headerTop' ,isFilter?'':'box-shadow']">
      <view class="searchWrap">
        <view class="search flex-between">
          <input type="text" v-model="keywords" placeholderClass="searchtext" placeholder="搜索患者姓名" />
          <uni-icons @click="search" type="search" class="searchIcon" size="20"></uni-icons>
        </view>
      </view>
    ...
    <view class="listWrap">
      <view class="listItem box-shadow" v-for="(item, index) in list" :key="index" @click="gotoInfo('/pages-doctor/user/prescription/detail?id='+(item.prescriptionNo||item.id))">
        <view class="list_a flex-between">
          <text style="width: 550rpx;white-space: nowrap;" class="overflow-hidden text-overflow">处方号：{{item.prescriptionNo||item.id}}</text>
          <text class="state state-blue" v-if="item.status == 'DRAFT'">草稿</text>
          <text class="state state-red" v-if="item.status == 'REJECTED'">已驳回</text>
          <text class="state state-blue" v-if="item.status == 'PENDING_REVIEW'">待审核</text>
          <text class="state state-green" v-if="item.status == 'PASSED'">已通过</text>
        </view>
        ...
        <view class="list_b flex-start-center">
          ...
          <text>{{item.serviceType=='CONSULT'?'在线咨询':item.serviceType=='REPEAT_CLINIC'?'在线复诊':'慢病续方'}}
            {{item.serviceMode=='GRAPHIC'?'图文':item.serviceMode=='VIDEO'?'视频':'电话'}}
          </text>
        </view>
        <view class="list_b flex-start-center">
        ...
        <view class="list_b flex-start-center">
          <text class="title">{{item.status == 'DRAFT'||item.status == 'REJECTED'?'创建时间':'提交时间' }}</text>
          <text class="value price">{{item.status == 'DRAFT'||item.status == 'REJECTED'?(item.createTime||''):(item.submitTime||'') }} </text>
        </view>
        ...
        <view class="flex-between list_c" @click.stop>
          ...
          <view class="flex-start-center">
            <button plain class="optBtn on" v-if="item.status=='PENDING_REVIEW'" @click='withdraw(item.prescriptionNo||item.id)'>撤回</button>
            <button plain class="optBtn" v-if="item.status=='REJECTED'" @click="cope(item)">复制</button>
            <button plain class="optBtn on" v-if="item.status=='DRAFT'" @click='del(item.prescriptionNo||item.id)'>删除</button>
            <button plain class="optBtn on" v-if="item.status=='DRAFT'"  @click="gotoInfo('/pages-doctor/user/prescription/detail?id='+(item.prescriptionNo||item.id))">修改</button>
            <button plain class="optBtn" v-if="item.status=='DRAFT'" @click="submit(item)">提交</button>
          </view>
        </view>
      </view>
    </view>
  ...
  export default {
    ...
    methods: {
      ...
      async cope(item) {
        ...
        let resp = await addAndEdit({id:this.newCode,rpDrugList:res.rpDrugList})
        console.log(resp,'-----------------------------------')
        if(resp){
          uni.showToast({
            title: '复制成功！',
