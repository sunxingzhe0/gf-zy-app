  ...
  <view class="orderWrap2">
        ...
        <scroll-view scroll-x class="tabList">
          <view class="tabWrap">
            <view
              ...
              @click="tabTap(0)"
              >全部({{ extend.all || 0 }})</view
            >
            <view
              :class="['tabItem', current == 1 ? 'on' : '']"
              @click="tabTap(1)"
              >待接诊({{ extend.waitTreat || 0 }})</view
            >
            <view
              :class="['tabItem', current == 2 ? 'on' : '']"
              @click="tabTap(2)"
              >接诊中({{ extend.inTreat || 0 }})</view
            >
            <view
              :class="['tabItem', current == 3 ? 'on' : '']"
              @click="tabTap(3)"
              >待评价({{ extend.toEvaluate || 0 }})</view
            >
            <view
              :class="['tabItem', current == 4 ? 'on' : '']"
              @click="tabTap(4)"
              >已完成({{ extend.finished || 0 }})</view
            >
            <view
              :class="['tabItem', current == 5 ? 'on' : '']"
              @click="tabTap(5)"
              >已关闭({{ extend.closed || 0 }})</view
            >
          </view>
        </scroll-view>
      </view>
    ...
    <scroll-view scroll-y="true" @scrolltolower="tolower" class="listWrap">
      <view
        ...
        :key="index"
        @click="
          gotoInfo('/pages-doctor/user/order/detail?orderId=' + item.orderId)
        "
      >
        <view class="list_a flex-between">
          ...
          <text class="state" v-if="item.orderStatus == 'TO_EVALUATE'"
            >待评价</text
          >
          <text class="state" v-if="item.orderStatus == 'FINISHED'"
            >已完成</text
          >
        ...
        <view class="list_b flex-start-center">
          ...
          <text class="value"
            >{{ switchBizType(item.bizType) }}
            {{ switchBizWay(item.wayType) }}</text
          >
        </view>
        <view class="list_b flex-start-center">
        ...
        <view class="countdown" v-if="item.orderStatus == 'WAIT_TREAT'"
          ><text>{{ countdown(item) }} 剩余：</text>
        ...
        <view class="flex-between list_c" @click.stop>
          ...
          <view class="flex-start-center">
            <button
              plain
              class="optBtn"
              v-if="item.option.accept"
              @click="
                gotoInfo('/pages-doctor/chat/chat?orderId=' + item.orderId)
              "
            >
              接诊
            </button>
            <button
              plain
              class="optBtn on"
              v-if="item.option.audit"
              @click="reject(false, item.orderId)"
            >
              拒绝
            </button>
            <button
              ...
              class="optBtn"
              v-if="item.option.audit"
              @click="reject(true, item.orderId)"
            >
              同意接诊
            </button>
            <button
            ...
            <button
              ...
              @click="
                gotoInfo(
                  '/pages-doctor/user/prescription/info?id=' + item.orderId,
                )
              "
            >
              处方
