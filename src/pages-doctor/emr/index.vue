<template>
  <view class="emrWrap">
    ...
    <app-card v-for="(item,index) in tabs" :key="index" styles="padding: 40rpx 30rpx;">
      <view class="title">{{item.label}}</view>
      <view class="content">
        <block v-if="item.name == 'dtoList'">
          <view v-for="(dtoItem, index) in emrData.dtoList" :key="dtoItem.id">
            <text>{{ index + 1 }}. {{ dtoItem.prefixName }}{{ dtoItem.diagnosisName }} {{ dtoItem.suffixName }} ({{dtoItem.mainDiagnosis === 1 ? '主诊断' : '诊断'}}){{ !dtoItem.childDtos.length ? '；' : '，' }}
            </text>
            <text v-for="child in dtoItem.childDtos" :key="child.indexCode">
              {{ child.prefixName }} {{ child.diagnosisName }} {{ child.suffixName }} (子诊断)；
            </text>
          </view>
        </block>
        <view v-else>
          <view v-if="emrData.diseId">{{emrData[item.name] || ''}}</view>
           <textarea v-show="!emrData.diseId && isItemText" class="text" :maxlength="item.name == 'mainSuit'?20:3000" v-model="emrData[item.name]" placeholder="请输入"/>
        </view>
      </view>
      <view class="editBtn flex-justify-end">
        <view @click="editRecordTap(item)" v-if="emrData.diseId">修改记录</view>
        <view class="on" @click="editTap(item)">{{emrData.diseId?'修改':item.name=='dtoList'?'编辑':'保存'}}</view>
      </view>
    </app-card>
    <view class="emrBtn flex-justify-end">
      <view class="typesItem" @click="templateTap">导入模板</view>
      <view class="typesItem" v-if="emrData.diseId" @click="editRecordTap()">提交记录</view>
    ...
    <uni-popup ref="edit">
      <view class="pop-wrap">
        <view class="pop-main">
          ...
          <view class="popText">
            <textarea v-show="istextarea" :maxlength="thatInfo.name == 'mainSuit'?20:3000" class="textMain" placeholder="请输入" placeholder-style="font-size:26rpx; color:#666;"
              v-model="content"></textarea>
            <!-- <view class="textNum">{{ content.length || 0 }}/200</view> -->
          </view>
        </view>
        <view class="pop-btn flex-between">
          <view class="close flex_1" @click="this.$refs.edit.close();istextarea = false">取消</view>
          <view class="flex_1" @click="save()">保存</view>
        </view>
      </view>
    </uni-popup>
    <!--修改记录-->
    <edit-record ref="editRecord" :title="thatInfo.label" :orderId="orderId" :type="thatInfo.type" :diseaseId="emrData.diseaseId"></edit-record>
    <!--导入模板-->
    <templet-list @popChange="popChange" ref="templetList" @importTem="importTem"></templet-list>
    <!--修改诊断-->
    <diagnosis @popChange="popChange" ref="diagnosis" :orderIdP="orderId" :memberIdP="memberId" @diagnosisChange="diagnosisChange"></diagnosis>
  </view>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async save(title) {
        ...
        await saveDiseaseTemp({ ...this.emrData,
          orderId: this.orderId
        })
      ...
      async submit() {
        if(this.isSubmit){
          this.isSubmit = false
          await submitDisease({ ...this.emrData,
            orderId: this.orderId,
