<template>
    ...
    <homeIcom />
		<view>
			<uni-swipe-action>
				<uni-swipe-action-item :options="options" v-for="(item, index) in list" :key="index" @click="del($event, item.id)">
					<view class="list box-shadow flex-between" @click="info(item,index)">
						<view :class="['listImg', item.readed ? 'read' : '']"><text class="iconfont icon-tongzhi"></text></view>
						<view class="info">
...
<script>
import { queryAnyData, delNotice, readNotice, batchRead } from '@/common/request/userCenter';
export default {
	data() {
		return {
			more: 'loading',
