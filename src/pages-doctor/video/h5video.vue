<template>
  <view>
    ...
    <!--  <live-pusher v-if="authorize && pushUrl" id="live-pusher" :class="!playUrl || screenWindowUser == 'doctor' ? 'screen-full' : 'small-window'"
      ...
      <live-player v-if="playUrl" :class="!playUrl || screenWindowUser == 'user' ? 'screen-full' : 'small-window'"
      ...
      <scroll-view class="message-box-container" :class="{ 'input-show': showKeyboard,'tool-bar':toolBarShow }"
        :scroll-y="true" :scroll-top="scrollTop" :scroll-anchoring="true"
        @scrolltoupper="getHistoryMessage">
        <view id="message-box" class="message-box" :class="{'tool-show':toolBarShow}">
          <video-message :messageBox="messageBox" :order="roomInfo"
      ...
      <view class="bottom-bar" v-show="showKeyboard">
        <view class="flex-between">
          <image v-if="roomInfo.status == 'IN_TREAT'" class="bottom-bar__icon"
            :src="require('@/assets/off38@2x.png')" mode="aspectFit" @click="finishService"></image>
          <input class="bottom-bar__input" type="text" :value="inputValue"
            placeholder="请输入您要咨询的问题" confirm-type="send" @input="inputChange"
            @confirm="inputConfirm" />
          <image class="bottom-bar__icon" :src="require('@/assets/gengduo@2x.png')"
            mode="aspectFit" @click="toolBarShow = !toolBarShow"></image>
          <image class="bottom-bar__icon" :src="require('@/assets/jianpan@2x.png')"
            mode="aspectFit" style="padding-left: 0;" @click="showKeyboard = false"></image>
        </view>
        <view class="bottom-bar__toolbar" v-show="toolBarShow">
          <view class="flex-start-start" v-show="!commonWordsShow">
            <view class="bottom-bar__tool_item" v-for="(item, index) in chatTools"
              :key="index" :data-event="item.event" @click="toolListener">
              <view class="app-card">
          ...
          <view class="common-words-wrapper" v-show="commonWordsShow">
            <view class="common-words__item" v-for="(item, index) in commonWords"
              :key="index" @click="selectCommonWords(index)">{{ item.content }}</view>
          </view>
        </view>
      </view>
      <view class="btn-suspended" :style="showKeyboard?'bottom:140rpx;height:88rpx;border-radius: 50%;padding: 0;':''">
        <image class="btn-suspended__icon" :src="require('@/assets/shutdown1@2x.png')"
          mode="aspectFit" @click="shutDown"></image>
        <image v-if="roomInfo.status == 'IN_TREAT' && !showKeyboard" class="btn-suspended__icon"
          :src="require('@/assets/off38@2x.png')" mode="aspectFit" @click="finishService"></image>
        <image class="btn-suspended__icon" v-if="!showKeyboard" :src="require('@/assets/jianpan2@2x.png')"
          mode="aspectFit" @click="showKeyboard = true"></image>
      </view>
    </view>
  </view>
</template>
  ...
  export default {
    ...
    onUnload() {
     ...
     switch (uni.getSystemInfoSync().platform) {
       ...
       case 'ios':
         window.webkit.messageHandlers.leaveVideo.postMessage(this.roomInfo.sessionId)
         break;
     }
     //#endif
      // this.client.leave()
    ...
    methods: {
     // agoraInit() {
      ...
      //         e => {
      //           console.log(
      //             `stream subscribed failed ${e} ${e.code} ${e.reason}`);
      //         }
      //       );
      //     },
      //     err => {
      ...
      //             e => {
      //               console.log(
      //                 `client publish failed: ${JSON.stringify(e)}`);
      //             }
      //           );
      //         },
      //         e => {
      //           console.log(
      //             `client join ${channel} failed: ${JSON.stringify(e)}`
      //           );
      //         }
      //       );
      //     },
      ...
      async getClinicInfo() {
        ...
        this.$nextTick(() => {
          ...
          switch (uni.getSystemInfoSync().platform) {
            ...
            case 'ios':
              window.webkit.messageHandlers.startVideo.postMessage(this.roomInfo.sessionId)
              break;
          }
          //#endif
     ...
     //  },
      ...
      async getHistoryMessage() {
        if (['APPOINTMENT', 'WAIT_TREAT'].includes(this.roomInfo.status))
          return;
        const params = {
          userId: this.userId,
