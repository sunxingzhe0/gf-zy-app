<template>
  <view class="date">
    ...
    <view v-show="isopen" class="dateTop flex-start-center">
      ...
      <image class="icon" :src="isopen ? require('@/assets/hide.png') : require('@/assets/show.png')"
        @click="isopen = !isopen"></image>
    </view>
    <view v-show="isopen" style="position: relative;">
      <view style="font-size: 24rpx;color: #999;position: absolute;right: 30rpx;top: 20rpx;z-index: 11;" @click="clearDate">清空</view>
      <uni-calendar ref="date" :insert="true" :range="true" :startDate="startDate"
        :showMonth="false" @monthSwitch="calendarInit" @change="changeCalendar" />
    </view>
    <view class="weekWrap" v-show="isopen">
      <scroll-view class="scroll-view_H" scroll-x="true" scroll-left="0">
        <view :class="['weekItem', item == isweek ? 'on' : '']" v-for="item in weekend"
          :key="item.day" @click="changeWeek(index)">
          <view>周{{ weeks[item.day] }}</view>
          <view class="time">{{ item.year }}-{{ item.month }}-{{ item.date }}</view>
        </view>
      </scroll-view>
    </view>
    <view class="plan_main" v-show="isopen">
      <view class="flex-between planShow paddt" v-for="item in rooms" :key="item.sessionId"
        :class="{'color-red': item.appointmentCountDown == 0}">
        <view class="flex-start-center plan-item">
          <view class="planTime">
            <view>{{ FORMATDATE(item.appointmentPeriod.split('~')[0], 'hh:mm') }}</view>
            <view>{{ item.appointmentPeriod.split('~')[1] }}</view>
          </view>
        ...
        <view class="planNum" v-if="item.appointmentCountDown > -1" :class="item.appointmentCountDown <= 10 ? 'color-red' : item.appointmentCountDown <= 30 ? 'color-yellow' : ''">
...
<script>
  export default {
    ...
    watch: {
      ...
      sessionList(list) {
        this.rooms = list.filter(({
          bizType,
          status
        }) => bizType && ['APPOINTMENT', 'WAIT_TREAT'].includes(status)).reverse();
      }
    },
    methods: {
      dateFormmat(_date) {
