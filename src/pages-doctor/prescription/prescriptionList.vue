  ...
  <view class="prescription-page">
    ...
    <view class="title-msg">
      ...
      <view class="diagnosis-time">
        ...
        <view class="show"
          ><button @click="showModal('痕迹查看')">痕迹查看</button></view
        >
      </view>
    </view>
    ...
    <view class="content">
      ...
      <view class="diagnosis">
        ...
        <view class="text" v-if="isOpen">
          <view v-for="(dtoItem, index) in diagContent" :key="dtoItem.id">
            <text
              >{{ index + 1 }}. {{ dtoItem.prefixName || '' }}
              {{ dtoItem.diagnosisName || '' }}
              {{ dtoItem.suffixName || '' }} ({{
                dtoItem.mainDiagnosis === 1 ? '主诊断' : '诊断'
              }}) {{ !dtoItem.childDtos.length ? '；' : '，' }}</text
            >
        ...
        <view class="botton-icon" @click="OpenTo"
          >{{ isOpen ? '收起' : '展开' }}
          <image
      ...
      <view class="prescription-list">
        ...
        <Item
          ref="rpItem"
          v-for="(val, index) in prescriptionListData"
          :key="val.id"
          :itemData="val"
          :orderInfo="orderInfo"
          :isShowSelect="isShowSelect"
          :isSelectCheckBox="isSelectCheckBox"
          @delprescription="delprescription"
          @delDrug="delDrug"
          @showModel="openModel"
          @withdraw="withdraw"
...
export default {
  ...
  async onLoad(option) {
    ...
    this.deptId = uni.getStorageSync('deptId') || orderInfo.biz.deptId
  ...
  methods: {
    async getListMsgs() {
      ...
      for (let i = 0; i < this.prescriptionListData.length; i++) {
        for (
          let j = 0;
          j < this.prescriptionListData[i].rpDrugList.length;
          j++
        ) {
          if (this.prescriptionListData[i].rpDrugList[j + 1] !== undefined) {
            if (
              this.prescriptionListData[i].rpDrugList[j + 1].groupId ===
                this.prescriptionListData[i].rpDrugList[j].groupId &&
              this.prescriptionListData[i].rpDrugList[j].groupId !== ''
            ) {
      ...
      for (let itm of this.prescriptionListData) {
        ...
        for (let vale of itm.rpDrugList) {
          // vale.drugTypeCode === 'WESTERN_MEDICINE'
          //   ? (itm.westPrice += +vale.totalPrice)
          //   : (itm.chPatentPrice += +vale.totalPrice)

          //总量单位枚举
          if (vale.split) {
            vale.alltotalUnit = {
              [vale.regularUnit]: vale.regularUnitText,
              [vale.basicUnit]: vale.basicUnitText,
            }
          } else {
    ...
    async hideDialog(data) {
      ...
      if (data === '保存') {
        ...
        if (this.isShowTemplate) {
            ...
            async success(res) {
              if (res.confirm) {
                ...
                resp.rpDrugList.forEach(item => {
                  ...
                  item.dosageUnit = item.singleDoseUnit
                  item.dosageUnitText = item.singleDoseUnitText
                })
                //小于五个药直接创建
                if (resp.rpDrugList.length <= 5) {
                  ...
                  //老处方的数据填到新处方
                  let response = await addAndEdit({
                    id: _this.newPresciptionCode,
                    rpDrugList: resp.rpDrugList,
                  })
                  _this.getListMsgs()
                } else {
                  //大于五个药 分方
          ...
          if (!(flag && this.params.drugMsg.totalUnit)) {
            uni.showToast({
              title:
                flag && !this.params.drugMsg.totalUnit
                  ? '请选择总量单位'
                  : '请检查输入内容',
              icon: 'none',
              duration: 1500,
          ...
          //通过药品id查询药房
          let resDatas = await drugInPharmacy({
            orgDrugId: this.params.drugMsg.id,
            deptId: this.deptId,
          })
          //药房列表
          ...
          //判断原药房id和新id是否一致
          if (
            nowData.rpDrugList[0]?.receivePharmacyId !==
              this.params.drugMsg.receivePharmacyId &&
            nowData.rpDrugList[0]?.receivePharmacyId
          ) {
            let _this = this
            //不一致
            uni.showModal({
              ...
              async success(res) {
                if (res.confirm) {
                  //添加到其他处方
                  //筛选出所有草稿处方
                  _this.allDraftList = _this.prescriptionListData.filter(
                    item => {
                      return item.status === 'DRAFT'
                    },
                  )
                  ...
                  for (let i = 0; i < _this.minfiveData.length; i++) {
                    for (
                      let j = 0;
                      j < _this.minfiveData[i].rpDrugList.length;
                      j++
                    ) {
                      _this.minfiveData[i].rpDrugList[j].receivePharmacyId ===
                        _this.params.drugMsg.receivePharmacyId &&
                        ((isExistencetow = true),
                        (pharmacyId = _this.minfiveData[i].id))
                      //有此药房的处方id
                    }
                  }
                  ...
                  if (isExistencetow) {
                    ...
                    _this.getListMsgs()
                    rpDrugList.forEach(item => {
                      item.dosageUnit = item.singleDoseUnit
                      item.dosageUnitText = item.singleDoseUnitText
                    })
                    let res = await addAndEdit({
                      id: pharmacyId,
                      rpDrugList: rpDrugList,
                    })
                    if (res) {
                      uni.showToast({
                        title: '保存成功',
            ...
            if (this.oldpreData.prescriptionId != this.params.id) {
              ...
              oldDatasList.forEach((item, index) => {
                item.id === this.preservationId &&
                  (oldDatasList[index] = this.params.drugMsg)
                item.dosageUnit = item.singleDoseUnit
                item.dosageUnitText = item.singleDoseUnitText
              })
    ...
    async addRpedit() {
      ...
      if (nowData.rpDrugList.length > 0) {
        ...
        if (nowId !== this.params.drugMsg.receivePharmacyId) {
                      ...
                      if (res.confirm) {
                        ...
                        for (let i = 0; i < _this.minfiveData.length; i++) {
                          for (
                            ...
                            _this.minfiveData[i].rpDrugList[j]
                              .receivePharmacyId ===
                              _this.params.drugMsg.receivePharmacyId &&
                              ((isExistencetow = true),
                              (pharmacyId = _this.minfiveData[i].id))
          ...
          console.log(
            JSON.stringify({ id: nowData.id, rpDrugList: nowData.rpDrugList }),
            '新增-----------------------',
          )
          //数据填到处方
          nowData.rpDrugList.forEach(item => {
            item.dosageUnit = item.singleDoseUnit
            item.dosageUnitText = item.singleDoseUnitText
          })
          let respq = await addAndEdit({
            id: nowData.id,
            rpDrugList: nowData.rpDrugList,
          })
          if (respq) {
            uni.showToast({
              title: '添加成功',
    ...
    async singleSubmit(data) {
      ...
      //处方提交判断是否需要授权
      try {
        let res = await submitRp({
          submit: [{ id: data.id, rpDrugList: data.rpDrugList }],
        })
        this.getListMsgs()
        uni.showToast({
          title: '提交成功',
    ...
    async copyPrescription(data) {
      ...
      let res = await addAndEdit({
        id: this.newPresciptionCode,
        rpDrugList: data.rpDrugList,
      })
      this.getListMsgs()
    },
    //点击开始的设为模板
    ...
    async hideSetModel(data) {
      ...
      if (data === '保存') {
        ...
        console.log(
          '设置模板时的数据---------',
          JSON.stringify(this.modelSetData),
        )
    ...
    changicSelect(val) {
      this.options.forEach(item => {
        val.newVal === item.diagnosisName &&
          ((this.modelSetData.icdName = item.diagnosisName),
          (this.modelSetData.icdId = item.id))
      })
    },
    //诊断关键字查询
    async inputSearch(e) {
