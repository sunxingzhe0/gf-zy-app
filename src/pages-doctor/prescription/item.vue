<template>
  <view class="item-page">
    ...
    <view class="top-msg">
      ...
      <view >
        <view class="code">
          ...
          <text :class="{draet:status.DRAFT==status[itemData.status],pass:status.PASSED==status[itemData.status],reject:status.REJECTED==status[itemData.status],pending:status.PENDING_REVIEW==status[itemData.status]}">{{status[itemData.status]}}</text>
        </view>
        <view class="btn-box" v-if="itemData.status!=='PASSED'">
          <text @click="isNext(autMap[itemData.status])" class="button" :class="{draet:autMap.DRAFT==autMap[itemData.status],pass:autMap.PASSED==autMap[itemData.status],reject:autMap.REJECTED==autMap[itemData.status],pending:autMap.PENDING_REVIEW==autMap[itemData.status]}">{{autMap[itemData.status]}}</text>
          <text v-if="status.DRAFT==status[itemData.status]" @click="singleSubmit" class="single-submit">提交</text>
        </view>
      </view>
      <!-- 驳回原因 -->
      ...
      <view >
        <view>
          <text class="num">数量：{{itemData.rpDrugList.length?itemData.rpDrugList.length:0}}</text>
        </view>
        <view class="num-doctor">
          <text>医生：{{orderInfo.doctorName+" "+orderInfo.titleName }}</text>
        </view>
        <view>
      ...
      <view >
        ...
        <view>
          <text class="total">合计：￥{{itemData.prescriptionPrice||Number(itemData.westPrice)+Number(itemData.chPatentPrice)}}</text>
        </view>
      </view>

      <!-- 4 -->
      <view class="group">
        <view class="pharmacy-name">接收方：{{itemData.rpDrugList[0].receivePharmacy||''}}</view>
        <view v-if="itemData.rpDrugList.length>1&&!isShowSelect">
            <button @click="delGrop(itemData.id)" v-if="isDelGroup" class="cancel">取消成组</button>
            <button @click="setGrop(itemData.id)" v-if="isShowGroup" class="confirm">成组</button>
        </view>
      </view>

      <!-- 5 -->
      <view class="trial-result" v-if="status.PASSED==status[itemData.status]||status.REJECTED==status[itemData.status]">
    ...
    <view class="drugs-lists" :class="{isHide:!isOpen}">
      <checkbox-group @change="checkChange">
        <view class="list" v-for="(val) in itemData.rpDrugList" :key="val.id">
          <!-- 1 -->
          <view>
            <text class="text-color">{{`${val.name}(规格：${val.spec})`}}</text>
          ...
          <!-- 2 -->
          <view>
            <text class="num-day">{{`${allchannel[val.useWays]} ${allfrequency[val.useFrequency]} ${val.singleDose}${val.basicUnitText}/次 &nbsp; ${val.treatment}${attrdate[val.treatmentUnit]||val.treatmentUnit}`}}</text>
            <text class="price-num">{{`￥${val.price}/${val.alltotalUnit[val.totalUnit]} x ${val.total} `}}</text>
          ...
          <view v-if="itemData.status=='DRAFT'||isShowSelect">
            <view>
                <checkbox :checked="val.checked" :disabled="val.isGetClass" :value="val.id" color="#08B4C8" style="transform:scale(0.7)"/>
            </view>
            <view class="btn-box" v-if="status.DRAFT==status[itemData.status]">
    ...
    <!-- 展开折叠 -->
    <view v-show="itemData.rpDrugList.length>1" class="botton-icon" @click="OpenTo">{{isOpen?"收起":"更多"}}
    ...
    methods:{
      ...
      async setGrop(val){
        ...
        if(this.checkedList.length<2){
          ...
          let res = await addAndEdit({id:val , rpDrugList:this.itemData.rpDrugList})
          if(res){
            uni.showToast({
              title:'成组成功',
              icon:'none',
           ...
           uni.showToast({
            ...
            uni.showModal({
              ...
              async success(res) {
                  ...
                  let res = await addAndEdit({id:val , rpDrugList:_this.itemData.rpDrugList})
                  if(res){
