<template>
  <view class="inspect-table">
    ...
    <view class="head" :key="inspent">
      ...
      <view class="num">共计<span style="color:#f66666">{{' '+one.list.length+' '}}</span>条</view>
    </view>
    <view class="collapse" 
      :style="two.isOpen?'height:auto':'height:140rpx'" 
      v-for="(two,labSno) in one.list" :key="labSno">
      <view class="number">
      ...
      <view class="texts">
        <view class="tips">
          科室：{{two.transBeanList[0].value||'-'}} | 
          标本：{{two.transBeanList[1].value||'-'}}</view>
        <view class="checks">
          置顶异常值
      ...
      <view class="result-table">
        <table cellspacing="0">
          ...
          <tbody>
            <tr v-for="(t,table) in two.tableList" :key="table">
              <td>{{t.itemCHName||'-'}}</td>
              <td>{{t.itemResultNum||'-'}}</td>
              <td>
        ...
        <view class="doctor-remark">
          <view class="doctor">申请医生：{{two.transBeanList[2].value||'-'}}</view>
          <view class="time">申请日期：{{two.transBeanList[5].value||'-'}}</view>
        </view>
        <view class="doctor-remark">
          <view class="doctor">审核医生：{{two.transBeanList[3].value||'-'}}</view>
          <view class="time">审核时间：{{two.transBeanList[7].value||'-'}}</view>
        </view>
        <view class="doctor-remark">
          <view class="doctor">报告医生：{{two.transBeanList[4].value||'-'}}</view>
          <view class="time">报告时间：{{two.transBeanList[8].value||'-'}}</view>
        </view>
        <view class="do-time">采集时间：{{two.transBeanList[6].value||'-'}}</view>
      </view>
    </view>
    </template>
  </view>
