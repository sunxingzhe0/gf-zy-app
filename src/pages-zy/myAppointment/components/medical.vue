<template>
  <view>
    <view class="menzhenItemBox box-shadow" v-if="type == 0" v-for="(item, index) in list" :key="index">
      ...
      <view class="contentBox flex-start-start">
        ...
        <view class="content flex_1">
          <view class="name flex-between">
            ...
            <text>{{item.orderState == 0?'已支付':item.orderState==1?'已退款':'-'}}</text>
          </view>
          ...
          <view class="doctor">诊查费：¥{{ parseFloat(item.fee).toFixed(2) }}</view>
          <view class="cancelBtn" v-if="item.cancel" @click.stop="showPop(item.recordId)">取消</view>
        </view>
      </view>
    </view>
    <view class="menzhenItemBox box-shadow" v-if="type == 1" v-for="(item, index) in list" :key="index" @click="goto(`/pages-zy/myAppointment/detail?orderId=${item.orderId}&recordId=${item.recordId}`)">

      <view class="contentBox">
        <view class="name flex-between">
          <view style="width: 400rpx;">{{item.packageName}}</view>
          <text>{{item.orderState == 0?'已退款':item.orderState==1?'已支付':'-'}}</text>
        </view>
        <view class="flex-between">
        ...
        <view class="cancelBtn" v-if="item.cancel && item.orderState==1" @click.stop="showPop(item.recordId)">取消预约</view>
        <view class="cancelBtn default" v-if="item.hasReport" @click.stop="goto(`/pages-zy/physical/report/detail?checkupNo=${item.reportNo}&patientId=${patientInfo.memberId}`)">体检报告</view>
      </view>
    </view>
