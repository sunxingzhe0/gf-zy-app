<template>
  <view style="padding-bottom:120rpx">
    <view>
      <view class="top-tip" v-if="info.cancel && info.orderState == 1"
        >温馨提示：取消预约请至少提前一天，否则当日无法取消。</view
      >
      <view
        class="top-bg text-center"
        :style="{
      ...
      >
        <view>{{
          info.orderState == 0 ? '已退款' : info.orderState == 1 ? '已支付' : ''
        }}</view>
      </view>
    </view>
    ...
    <view class="panWrap">
      ...
      <app-card styles="padding: 30rpx;">
        ...
        <app-list-item
          label="支付方式"
          :styles="{ padding: '10rpx', paddingLeft: 0 }"
          :value="info.payWayName"
          noBorder
        ></app-list-item>
        <app-list-item
          label="支付金额"
          :styles="{ padding: '10rpx', paddingLeft: 0 }"
          :value="`￥${parseFloat(info.payFee).toFixed(2)}`"
          noBorder
        ></app-list-item>
        <app-list-item
          label="支付时间"
        ...
        <app-list-item
          ...
          v-if="info.refundFee"
          :styles="{ padding: '10rpx', paddingLeft: 0 }"
          :value="`￥${parseFloat(info.refundFee).toFixed(2)}`"
          noBorder
        ></app-list-item>
        <app-list-item
          label="退费方式"
          v-if="info.refundWayName"
          :styles="{ padding: '10rpx', paddingLeft: 0 }"
          :value="info.refundWayName"
          noBorder
        ></app-list-item>
        <app-list-item
          label="退费时间"
      ...
      <app-card styles="padding: 0;">
        ...
        <view class="typeProject">
          ...
          <view
            class="projectItem"
            v-for="(item, index) in info.itemInfo"
            :key="index"
          >
            <view class="title">
              {{ item.combName }}
            ...
            <view class="content">
              <view class="name" v-if="item.combDetail">{{
                item.combDetail
              }}</view>
    ...
    <view
      class="fiexdBtn flex-between"
      v-if="info.orderState == 1 && info.cancel"
    >
      <view class="font32 color666">
        费用金额：<span class="price"
          >￥{{ parseFloat(info.fee).toFixed(2) }}</span
        >
      </view>
