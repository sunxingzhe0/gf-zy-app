        ...
        <view>
          <template v-for="(lin, t) in list">
            <view class="date" :key="t">
              {{ dayjs().format('YYYY') == lin.payTime.split('-')[0] ? '' : lin.payTime.split('-')[0] + '年'
                  }}{{
                    dayjs().format('YYYY-MM') == lin.payTime.substring(0, lin.payTime.lastIndexOf('-'))
                      ? '本月'
                      : lin.payTime.split('-')[1] + '月'
                  }}
            </view>
            <view class="box-shadow listWrap">
              <view class="list" v-for="(item, index) in lin.list" :key="index">
                <view class="flex-between listTop">
                  <view class="info">支付号：{{ item.payId }}</view>
                  <view :class="['state', type == 1 ? 'down' : '']">{{ type == 1 ? '线下' : '线上' }}</view>
                </view>
                <view class="listContent">
                  <view class="flex-between">
                    <view class="name">{{ item.patientName }}</view>
                    <view class="price">￥{{ parseFloat(item.fee).toFixed(2) }}</view>
                  </view>
                  <view class="info">
                    {{ item.inDept || ''}}
                  </view>
              ...
              <view class="list " v-for="(item, index) in lin.list" :key="index">
                ...
                <view class="listContent">
                  <view class="flex-between">
                    ...
                    <view class="price">￥{{ parseFloat(item.totalAmount).toFixed(2) }}</view>
                  </view>
                  <view class="info">
                    {{ item.deptName || ''}}
                  </view>
    ...
    methods: {
      ...
      listRecombination(labels) {
        ...
        for (var i = 0; i < labels.length; i++) {
          ...
          factory.showTime = factory.payTime ? dayjs(factory.payTime).format('YYYY-MM') : dayjs(factory.time).format(
            'YYYY-MM')
