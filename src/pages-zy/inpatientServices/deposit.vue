  ...
  <view style="padding: 20rpx; padding-top: 190rpx;">
    ...
    <topbar :tabs="[]">
      <template v-slot:top>
        ...
        <app-list-item label="入院日期" noBorder :styles="{padding:'16rpx 30rpx'}">
          <view :class="['flex_1 text-right',infoData.admitDtime? 'color666':'color999' ]">{{infoData.admitDtime || '暂无住院'}}</view>
        </app-list-item>
      </template>
    </topbar>
    <app-card class="moneyBox" styles="padding:30rpx;">
      ...
      <view class="nums">
        <view v-for="(item, index) in money" :key="index" :class="isActive == index ? 'numActive' : ''" @click="selMoney(item, index)">
          ￥ {{ item }}
        </view>
      </view>
      <view class="input">
        <input v-model="inputMoney" type="number" @blur="inputBlur" placeholder="请输入大于一千元的整百金额" />
      </view>
      <!-- 立即充值 -->
      <view class="recharge">
        <view :class="['confirmButton',isPay?'disabled':'']" @click="showPopup">立即充值</view>
        <view class="pricolor font24 mt-20" @click="goto">充值记录</view>
      </view>
    </app-card>
    <!-- 支付列表 -->
    <pop-pay ref="pay" :type="2" @cancel="payErr" @success="paysuccess" @payTap="pay">
      <view class="payInfo">
        ...
        <view class="inhosDate flex-start-center">
          ...
          <view class="date price">￥{{ parseFloat(putMoney).toFixed(2)}}</view>
        </view>
      </view>
    </pop-pay>
  </view>
  ...
  export default {
    ...
    methods: {
      ...
      pay(e) {
        ...
        rechargeIh({
            patientId: this.perId,
            fee: parseFloat(this.putMoney) * 100,
            payment: e
          })
          .then(data => {
