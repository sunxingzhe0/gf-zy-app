        ...
        <view class="listWrap">
          <view
            ...
            @click="
              goto(
                active == 0
                  ? `/pages-zy/outpatientPayment/detail?patientId=${perId}&type=${item.payState}&billNo=${item.prescNo}&visitNo=${item.visitNo}&title=${item.title}`
                  : active == 2
                  ? `/pages-zy/physical/payment/detail?patientId=${perId}&type=${item.payState}&billNo=${item.visitNo}&title=${item.title}`
                  : '',
              )
            "
          >
            <view class="dateBox flex-between">
              ...
              <view>
                <view v-if="item.payWay" class="payType">{{
                  item.payWay
                }}</view>
              </view>
            </view>
        ...
        <view class="listWrap">
          <view
            ...
            @click="
              goto(
                active == 'y-0'
                  ? `/pages-user/myAppointment/detail?id=${item.id}`
                  : `/pages-zy/myAppointment/detail?orderId=${item.id}&recordId=${item.orderNo}`,
              )
            "
          >
            <view class="dateBox flex-between">
            ...
            <view class="contentBox">
              <view class="name flex-between">
                ...
                <view class="payNum"
                  ><!-- {{ item.tag }} -->￥{{
                    parseFloat(item.totalMoney).toFixed(2)
                  }}</view
                >
              </view>
...
import { otherData, appointmentData } from '../api/user.js'
export default {
  components: {
    selectPatient,
    selectDateRange,
  ...
  methods: {
    ...
    changePatient(item) {
      console.log(item, 'itemohter')
      this.perId = item.memberId
      this.isotherPatient = item.otherPatient
      if (item.otherPatient) {
        this.perId = item.id
      }
    ...
    async getOtherData() {
      ...
      let list = await otherData({
        ...
        otherPatient: this.isotherPatient ? 1 : 0,
      })
      uni.hideLoading()
      this.testList1 = list
    },
