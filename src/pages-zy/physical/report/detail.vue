  ...
  <view style="padding: 20rpx;padding-bottom: 130rpx;">
    ...
    <view class="backgroundImg box-shadow mt-20">
      ...
      <view class="flex-between perInfo">
        <view
          >就诊人：<text>{{ info.patientName }}</text></view
        >
        <view
          >性别：<text>{{ info.gender }}</text></view
        >
        <view
          >年龄：<text>{{ info.age }}</text></view
        >
      </view>
    </view>
    <!-- 体检结果 tab 列表 -->
        ...
        <view style="padding-left: 30rpx;">
          ...
          >
            <view class="abnormal flex-between flex_1">
              ...
              <view class="icon">
                <view v-if="item.errNum > 0" class="text"
                  >{{ item.errNum }}项重点关注</view
                >
              </view>
            </view>
            ...
            <view class="abnormalData" v-if="item.errNum > 0" :key="index">
              ...
              <template v-for="(child, t) in item.details">
                <view class="abnormalItem" v-if="child.isErr" :key="index">
                  <view class="itenName"
                    >{{ child.itemName }}（{{ child.resultInterpre }}）</view
                  >
                  <view class="flex-between">
                    <view class="percent"
                      >范围:{{ child.resultRef
                      }}{{ child.resultUnit || '' }}</view
                    >
                    <view
                      >结果:{{ child.resultNum
                      }}{{ child.resultUnit || '' }}</view
...
<script>
import { tjResultItem, tjReportInfo, getAnalysis } from '../../api/physical.js'
export default {
  data() {
    return {
      // 切换激活
  ...
  methods: {
    ...
    goTo(item) {
      uni.navigateTo({
        url: `/pages-zy/physical/report/itemInfo?data=${encodeURIComponent(
          JSON.stringify(item),
        )}`,
      })
    },
