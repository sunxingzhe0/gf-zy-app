<template>
  <view :style="{paddingTop:'170rpx',paddingBottom:current==0?'120rpx':'20rpx'}">
    ...
    <view style="padding: 20rpx;">
      ...
      <template v-if="current == 0">
        <checkbox-group @change="checkboxChange">
          <view class="toPayBox box-shadow" v-for="(item, index) in waitPayList" :key="index">
            <view class="listBt">
              <checkbox class="checkbox" style="transform: scale(0.7);" color="#0ab2c1" :value="item.checkupNo" :checked="item.checked" />单据号:{{ item.bookingNo }}</view>
            <view class="contentBox">
              <view class="contentTitle">
                <view class="payTitle" @click="goto(`/pages-zy/physical/payment/detail?patientId=${perId}&type=0&billNo=${item.checkupNo}&title=${item.title}`)">{{ item.packageName }}<text>详情</text></view>
              </view>
              <view class="flex-between">
                <view>
                  <view class="createTime">体检时间：{{ item.regDate }}</view>
                </view>
                <view class="price">￥{{parseFloat(item.payFee).toFixed(2)}}</view>
              </view>
            </view>
          </view>
        ...
        <view class="fiexdBtn flex-between font30" v-if="waitPayList.length > 0 && current == 0">
          <checkbox-group style="width: auto;" @change="allCheckboxChange">
            <label>
              <checkbox style="transform: scale(0.7);" class="checkbox" color="#0ab2c1" value="isAll" :checked="isAll" />全选
            </label>
          </checkbox-group>
      ...
      <template v-if="current == 1 || current ==2 ">
        <view class="finishList box-shadow toPayBox" v-for="(item, index) in waitPayList" :key="index">
          <view class="listBt">单据号:{{ item.checkupNo }}</view>
          <view class="contentBox mt-20">
            <view class="contentTitle">
              <view class="payTitle" @click="goto(`/pages-zy/physical/payment/detail?patientId=${perId}&type=${current}&billNo=${item.checkupNo}&title=${item.title}`)">{{ item.packageName }}<text>详情</text></view>
            </view>
            <view class="flex-between">
              <view>
                <view class="createTime">体检时间：{{ item.regDate }}</view>
                <view class="createTime">{{current==1?'缴费':'退费'}}时间：{{ item.payTime }}</view>
              </view>
              <view class="price">￥{{parseFloat(item.payFee).toFixed(2)}}</view>
            </view>
          </view>
    ...
    <!-- 支付列表 -->
    <pop-pay ref="pay" :type="true" @success="paySucces" @payTap="paySubmit"></pop-pay>
  </view>
</template>
