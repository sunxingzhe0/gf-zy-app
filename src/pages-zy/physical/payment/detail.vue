    ...
    <view style="padding: 20rpx;margin-top: -40rpx;">
      ...
      <app-card class="panWrap" styles="padding: 30rpx;">
        ...
        <app-list-item
          label="支付金额"
          :styles="{ padding: '10rpx', paddingLeft: 0 }"
          :value="info.payFee ? `￥${parseFloat(info.payFee).toFixed(2)}` : '-'"
          noBorder
        ></app-list-item>
        <app-list-item
          label="支付时间"
      ...
      <app-card styles="padding: 0;">
        ...
        >
          ...
          <view class="objInfo" v-if="item.combDetail">{{
            item.combDetail
          }}</view>
    ...
    <view class="fiexdBtn flex-between" v-if="type == 0">
      <view class="font30 color666"
        >费用金额：<text class="colorred font-weight"
          >￥{{ info.payFee ? parseFloat(info.payFee).toFixed(2) : '-' }}</text
        ></view
      >
