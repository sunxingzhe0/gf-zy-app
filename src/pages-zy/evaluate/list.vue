          ...
          <view class="titleBox">
            <view class="title">
              <view class="left flex-start-center">
                <view class="type">{{
                  item.type == 0 ? '门诊' : item.type == 1 ? '住院' : '体检'
                }}</view>
          ...
          <view class="rateBox">
            <view class="itemBox">
              <view class="mt-10">
                <view class="textarea" v-if="!item.hasEva">
                  <textarea
                    ...
                    placeholder="非常好，我非常满意！"
                  />
                  <view class="tip"
                    >{{ item.content ? item.content.length : 0 }}/200</view
                  >
                </view>
              ...
              <view class="flex-start-center">
                ...
                <view class="star">
                  <uni-rate
                    :value="item.diagResult"
                    :size="15"
                    :margin="8"
                    color="#CCCCCC"
                    @change="starChange($event, item, 'diagResult')"
                    active-color="#FFA200"
                    :readonly="item.hasEva ? true : false"
                  />
                </view>
                <view class="rateFont">{{
                  getevaluateState(item.diagResult)
                }}</view>
              </view>
              <view class="mt-10 flex-start-center">
                ...
                <view class="star">
                  <uni-rate
                    :value="item.serviceAttitude"
                    :size="15"
                    :margin="8"
                    color="#CCCCCC"
                    active-color="#FFA200"
                    :readonly="item.hasEva ? true : false"
                    @change="starChange($event, item, 'serviceAttitude')"
                  />
                </view>
                <view class="rateFont">{{
                  getevaluateState(item.serviceAttitude)
                }}</view>
              </view>
              <view class="mt-10 flex-start-center">
                ...
                <view class="star">
                  <uni-rate
                    :value="item.hospitalEnvironment"
                    :size="15"
                    :margin="8"
                    color="#CCCCCC"
                    active-color="#FFA200"
                    :readonly="item.hasEva ? true : false"
                    @change="starChange($event, item, 'hospitalEnvironment')"
                  />
                </view>
                <view class="rateFont">{{
                  getevaluateState(item.hospitalEnvironment)
                }}</view>
              </view>
            </view>
          </view>
        ...
        >
          <view class="flex-between listTop" @click="goto(item.orderId)">
            <view class="flex-start-center">
              ...
              <view>
                ...
                <view class="name" style="line-height: 30rpx;"
                  ><text style="margin-left: 0;font-size: 22rpx;"
                    >订单号：{{ item.orderId }}</text
                  ></view
                >
              </view>
            ...
            <view class="flex-start-start" style="margin-bottom: 30rpx;">
              ...
              <view class="info">
                <view class="des flex-start-center evaluateNum">
                  ...
                  />
                  <text class="fen"
                    >{{ parseFloat(item.average).toFixed(1) }}分</text
                  >
          ...
          <view v-else class="flex-start-start" style="padding: 0 30rpx;">
            ...
            <view
              ...
              :style="
                item.answerContent
                  ? 'border-bottom: 1px solid #e6e6e6; padding-bottom:20rpx;'
                  : ''
              "
            >
              <view class="des flex-start-center evaluateNum">
...
import { findEvaluateListZy, addEvaluatezy } from '../api/user.js'
...
export default {
  ...
  methods: {
    ...
    goto(id) {
      uni.navigateTo({
        url: '/pages-user/serviceOrder/detail?bizId=' + id,
      })
    },
