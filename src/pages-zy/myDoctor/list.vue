<template>
  <view style="padding-top: 266rpx;">
    <!-- 收到视频聊天邀请 -->
    <video-invitation />
    <!--主页按钮-->
    <homeIcom />
    <view class="headerTop box-shadow">
      <app-list-item label="就诊人" :styles="{padding:'16rpx 20rpx',paddingLeft:0}">
      ...
      <view style="padding-right: 20rpx;">
        <view class="search flex-between">
          <input type="text" class="flex_1" v-model="keywords" placeholderClass="searchtext"
        ...
        <view class="tabList flex-between">
          <view class="tabWrap flex-between flex_1">
            <view :class="['tabItem', bizType == item.value ? 'on' : '']"
              @click="tabTap(item.value)" v-for="(item, index) in tabList" :key="index">{{ item.name }}</view>
          </view>
          <view class="iconBorder" @click="sortChange">
            <image :src="require('@/assets/paixu(1)@2x.png')" style="width: 30rpx;height: 30rpx;"></image>
          </view>
        </view>
      </view>
    ...
    <view class="main">
      ...
      <view class="docBox" v-if="bizType == 1">
        <view class="docItem box-shadow" v-for="(item, index) in docList" :key="index"
          @click="goto(item)">
          <view class="ptr">
            <default-img :url="FILE_URL(item.avatar)"></default-img>
          </view>
          <view class="contentBox">
            <view class="name">{{ item.name }} <text>{{item.sex == 1?'男':'女'}}</text></view>
            <view class="hos text-overflow">{{item.deptName || ''}}
              {{item.titleName || ''}} {{ item.orgName || ''}}</view>
            <view class="good mt-10 text-overflow">擅长：{{ item.skilled || '-' }}</view>
          </view>
        </view>
      </view>
      <view class="docBox" v-else>
        <view class="docItem box-shadow" v-for="(item, index) in docList" :key="index"
          ...
          <view class="contentBox">
            <view class="name">{{ item.doctorName }} <text>{{item.sex}}</text></view>
            <view class="hos text-overflow">{{item.deptName}} {{item.titleName}}
              {{ userInfo.orgName }}</view>
            <view class="good mt-10 text-overflow">擅长：{{ item.skilled || '-' }}</view>
            <view class="good mt-10">业务：{{ item.bizType == 'CONSULT' ? '在线咨询' : item.bizType == 'REPEAT_CLINIC' ? '在线复诊' : item.bizType == 'CARRYON_PRESC' ? '慢病续方' : '' }}</view>
            <view class="good mt-10">评分：<text class="fen">{{ item.avgScore || 0 }}</text>
  ...
  export default {
    ...
    methods: {
      ...
      goto(row) {
        if (this.bizType == 1) {
          uni.navigateTo({
            url: `/pages-zy/appointmentDoc/appointmentInfo?docId=${row.id}&deptId=${row.deptId}&orgId=${row.orgCode}`
          })
        } else {
          h5DocBizInfo({
            ...
            if (i == res.length - 1) {
              ...
              for (let f = 0; f < list.length; f++) {
                if (list[f].hasOpen && list[f].describe == 'CONSULT') {
                  ...
                  if (list[f].hasOpen && list[f].describe ==
                    'REPEAT_CLINIC') {
                    uni.navigateTo({
