<template>
  <view class="container">
    ...
    <view class="docBox">
      <view
        class="docItem flex-between box-shadow"
        v-for="(item, index) in starList"
        :key="index"
        @click="
          goTo(
            `/pages-zy/appointmentDoc/appointmentInfo?docId=${item.id}&deptId=${
              item.deptId
            }&orgId=${item.orgCode}&item=${JSON.stringify(item)}&starIn=1`,
          )
        "
      >
        ...
        <view class="contentBox">
          <view class="name"
            >{{ item.name }} <text>{{ item.sex || '' }}</text></view
          >
          <text class="info text-overflow"
...
import { startExperts } from '../api/user.js'
