<template>
  <view :class="{'scroll-fixed': !pageScroll}">
    ...
    <homeIcom />
    <view class="title">
      <user-search placeholder="搜索姓名/职称/擅长" @search="search()"></user-search>
      <view class="shaixuan">
    ...
    <view class="scroll">
      <view v-for="(item, index) in datas" class="item" :key="index" @click="to(item.doctorId)">
        <default-img class="item_image" :url="FILE_URL(item.picId)"></default-img>
        <view style="flex: 1;">
          <view>
          ...
          <view class="good">
            <text>擅长：</text>
            <text class="text-overflow overflow-hidden " style="max-width: 480rpx; flex: 1; white-space: nowrap;">{{ item.goodAt }}</text>
          </view>
          ...
          <view class="operation">
            <view class="open" :class="{ close: infoDto.state == 0 }" v-for="(infoDto, i) in item.infoDtos" :key="i">{{ infoDto.bizWayDesc }}:{{ infoDto.bizInfo }}</view>
          </view>
        </view>
      </view>
    ...
    <uni-drawer class="c-drawer-layout" ref="drawer" mode="right" @open="pageScroll = false" @close="pageScroll = true">
      <view class="drawer-scroll">
        <view class="drawer-title">选择业务类型</view>
        <view class="drawer-type">
          <text class="select-text " v-if="!isAppoint" :class="{ s: temporary_type == 'CONSULT' }" @click="temporary_type = 'CONSULT'">在线咨询</text>
        ...
        <view class="doctor-title">
          <view class="select-text" :class="{ s: temporary_titleAll }" @click="titleAllC()">
            <text>全部</text>
          </view>
          <view v-for="(item, index) in temporary_titleData" class="select-text" :class="{ s: item.state }" :key="index"
            @click="titleC(item)">
            <text>{{ item.name }}</text>
          </view>
        </view>
        ...
        <view class="select-text" style="width: 160rpx;" :class="{ s: temporary_departmentAll }" @click="departmentAllC()">全部</view>

        <view class="line"></view>
        <DeptList v-if="temporary_departmentData.length>0" :list="temporary_departmentData"
          @deptChange="deptChange"></DeptList>
        <!-- <uni-collapse>
          <uni-collapse-item
  ...
  export default {
    ...
    methods: {
      ...
      updateDept(arr) {
        return arr.map(item => {
          if (item.next) {
            this.updateDept(item.deptInner)
          } else {
      ...
      to(id) {
        ...
        uni.navigateTo({
          url: '../doctordetail/DoctorDetail?type=' +
...
<style lang="scss" scoped>
  ...
  /deep/.uni-collapse-cell__title.data-v-05a53ffd {
