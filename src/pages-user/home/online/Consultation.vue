      ...
      >
        ...
        <view style="flex: 1;">
          ...
          <view class="item-score">
            ...
            <text class="frequency">排队人数：{{ item.queueNum || '0' }} </text>
          </view>
    ...
    >
      <view class="drawer-scroll">
        ...
        <view class="drawer-type">
          ...
          <!-- <text
            class="select-text "
            :class="{ s: temporary_type == 'REPEAT_CLINIC' }"
            style="color:#ccc"
            >在线复诊</text
          >
          <text
            class="select-text "
            v-if="!isAppoint"
            :class="{ s: temporary_type == 'CARRYON_PRESC' }"
            style="color:#ccc"
            >慢病续方</text
          > -->
          <text
            class="select-text "
            :class="{ s: temporary_type == 'REPEAT_CLINIC' }"
            @click="temporary_type = 'REPEAT_CLINIC'"
            >在线复诊</text
          >
          <text
            class="select-text "
            v-if="!isAppoint"
            :class="{ s: temporary_type == 'CARRYON_PRESC' }"
            @click="temporary_type = 'CARRYON_PRESC'"
            >慢病续方</text
          >
        </view>
        ...
        <uni-collapse>
          <uni-collapse-item
            ...
            :key="index"
          >
            <view class="doctor-title dept-title">
              <view
                class="select-text"
...
export default {
  ...
  onLoad(option) {
    this.status = option.status //当前过滤状态 text video
    if (option.type) {
  ...
  methods: {
    ...
    getBizTypeList() {
      ...
      bizTypeList(this.parameter).then(data => {
        if (data) {
          const listData = this.datas.concat(data.data)
          //筛选条件新增------
          const textList = listData.filter(v =>
            v.infoDtos.some(it => it.state == 1 && it.bizWay == 'GRAPHIC'),
          )
          const videoList = listData.filter(v =>
            v.infoDtos.some(it => it.state == 1 && it.bizWay == 'VIDEO'),
          )
          this.datas =
            this.status === 'text'
              ? textList
    ...
    getDeptChooseList() {
      deptOuterChooseList({
        ...
        if (data) {
          ...
          if (this.parameter.deptIds.length > 0) {
            this.departmentData.forEach(item => {
              item.state = false
              if (this.parameter.deptIds.indexOf(item.id) > -1) {
                item.state = true
              } else {
                item.next &&
                  item.next.forEach(t => {
                    t.state = false
                    if (this.parameter.deptIds.indexOf(t.id) > -1) {
                      t.state = true
                    }
                  })
              }
    ...
    confirm() {
      ...
      uni.setNavigationBarTitle({
        title: types[this.temporary_type],
      })
    },
    to(id) {
      if (this.back) {
