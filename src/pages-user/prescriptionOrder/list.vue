  ...
  <view>
    ...
    <view class="listWrap">
      <view
        ...
        @click="
          gotoInfo(
            `/pages-user/prescriptionOrder/detail?bizId=${item.orderId}&expressInfoId=${item.expressInfo.id}`,
            item.orderStatus,
          )
        "
      >
        <view class="list_a flex-between" v-if="current != 'UNPAID'">
          ...
          <text
            :class="
              item.orderStatus == 'FINISHED' || item.orderStatus == 'CLOSED'
                ? ''
                : 'state'
            "
            >{{ getstate(item.orderStatus) }}</text
          >
        </view>
        <view v-else class="list_a flex-between">
          <text>服务订单号：{{ item.orderId }}</text>
        </view>
        <!--  <view class="rp-icon" v-if="current != 'UNPAID'">
          <text @click.stop="openPdf(item, 0)" class="rp-msg">处方签</text>
        </view> -->
        <view v-if="current != 'UNPAID'">
          <view
            class="list_a flex-between"
            v-if="item.orderStatus == 'WAIT_RECEIVE'"
            @click="logisticsInfo"
          >
            <image
              class="textIcon"
            ...
            <view class="receivedInfo">
              <view class="text-overflow"
                >【{{ item.expressInfo.province || '暂无物流信息' }}】{{
                  item.expressInfo.content || ''
                }}</view
              >
              <text class="">{{ item.expressInfo.update_time }}</text>
            </view>
            <uni-icons
              v-if="item.expressInfo.province"
              type="arrowright"
              class="rightIcon"
            ></uni-icons>
          </view>
          <view class="list_b flex-start-center">
            ...
            <text style="color:red">¥{{ item.payFee || '-' }}</text>
          </view>
          <view class="list_b flex-start-center">
            <text class="title">开方医生</text>
            <text class="value">{{
              item.doctorName + ' ' + item.titleName + ' ' + item.deptName
            }}</text>
          </view>
          <view class="list_b flex-start-center">
          ...
          <view class="flex-between list_c">
            ...
            <view class="flex-start-center" @click.stop>
              ...
              <button
                plain
                class="optBtn on"
                v-if="item.option.showExpress"
                @click="
                  gotoInfo(
                    '/pages-user/logistics/logistics?id=' + item.expressInfo.id,
                    item.orderStatus,
                  )
                "
              >
              ...
              <button
                plain
                class="optBtn"
                v-if="item.option.confirm"
                @click="operation(item.orderId, 'confirm')"
              >
                确认收货
              </button>
              <button
                ...
                class="optBtn"
                v-if="
                  item.option.take || getstate(item.orderStatus) === '待发货'
                "
                @click="rpRefund(item)"
              >
                退款
              ...
              <button
                plain
                class="optBtn"
                v-if="item.option.take"
                @click="selfExtracting(item.orderId)"
              >
                自提码
              </button>
              <button
                ...
                @click="operation(item.orderId, 'cancel')"
              >
                取消
              </button>
              <button
                ...
                @click="operation(item.orderId, 'delete')"
              >
                删除
              </button>
            </view>
        ...
        <view v-else>
          ...
          >
            <view class="prescription flex-between">
              <view class="prescriptionTitle" @click.stop>
                <evan-checkbox
                  ...
                  @change="checkboxChange(item, r)"
                >
                  <text class="checkText">处方号：{{ r.id }}</text>
                </evan-checkbox>
              </view>
              <text @click.stop="openPdf(r, 'UNPAID')" class="rp-msg"
                >处方签</text
              >
            ...
            <view class="flex-between">
              ...
              <view
                @click.stop
                class="more color666 font24"
                @click="tapOpen(index, p)"
              >
                {{ !r.isOpen ? '查看' : '收起' }}详情
                <image
                  :class="!r.isOpen ? 'moreUp' : 'moreDown'"
              ...
              <view class="recipelItem" v-for="(f, i) in r.rpDrugList" :key="i">
                <view class="flex-between">
                  ...
                  <view class="font26 color666"
                    >医保：{{ swithMedicare(f.medicare) }}</view
                  >
                </view>
          ...
          <view
            class="flex-between"
            style="padding-right: 30rpx; padding-top: 15rpx;"
            @click.stop
          >
            <evan-checkbox
              :value="item.isAll"
              shape="square"
              :primary-color="primaryColor"
              @change="allChange(index, item)"
              ><text class="checkText">全选</text></evan-checkbox
            >
            <view class="flex-start-center">
              <view class="font30 pricolor"
                >￥{{ item.totalPrice || '0.00' }}</view
              >
    ...
    <uni-popup ref="selfExtracting" type="bottom">
      <view class="selfExtracting">
        ...
        <view
          class="store "
          v-for="(item, index) in codeInfo.takeInfo"
          :key="index"
        >
...
export default {
  ...
  methods: {
    ...
    async openPdf(item, type) {
      console.log(item, '处方信息')
    ...
    checkboxChange(item, check) {
      ...
      this.$set(
        item,
        'ischeck',
        item.rpList.filter(item => item.checked).map(t => t.id),
      )
      this.$set(
        item,
        'isAll',
        item.ischeck.length == item.rpList.length && item.ischeck.length > 0,
      )
      this.getPrice(item)
    },
    getPrice(item) {
    ...
    pay(index) {
      ...
      uni.navigateTo({
        url:
          '/pages-user/orderSubmit/orderSubmit?ids=' +
          this.list[index].ischeck +
          '&total=' +
          this.list[index].totalPrice,
      })
    },
    // 展开药品
    tapOpen(index, p) {
      this.$set(
        this.list[index].rpList[p],
        'isOpen',
        !this.list[index].rpList[p].isOpen,
      )
    },
    /***
     *付款时间倒计时
...
.listWrap .listItem .list_b .title {
  flex-shrink: 0;
}
.rp-icon {
  display: flex;
