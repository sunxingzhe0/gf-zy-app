  ...
  <view class="container">
      ...
      <uni-popup-dialog>
        <view class="dialog-fade">
          <view class="dialog-body">
            <view class="close-icon" @click="closePointer"><icon type="clear" :size="18"></icon></view>
            <view class="dialog-title">自提点</view>
            <view class="flex-start-center">
              <view style="color: #666;">自提点</view>
              <view class="ml-20">{{ details.storeDtoList[selectSelfIndex].pharmacyName }}</view>
            </view>
            <view class="flex-start-center  mt-20">
              <view style="color: #666;">联系电话</view>
              <view class="ml-20">{{ details.storeDtoList[selectSelfIndex].pharmacyPhone }}</view>
            </view>
            <view class="flex-start-center mt-20">
              <view style="color: #666;">详细地址</view>
              <view class="ml-20 ">{{ details.storeDtoList[selectSelfIndex].address }}</view>
            </view>
          </view>
        </view>
      </uni-popup-dialog>
    ...
    <view class="app-card" v-if="active == 'self'">
      <view v-for="(item, index) in details.storeDtoList" :key="index">
        <view class="flex-between cell" @click="viewPointer(index)">
          ...
          <view><image class="arrow-horizontal" :src="require('@/assets/right11@2x.png')" mode="aspectFit"></image></view>
        </view>
        <view class="flex-between cell" @click="selectTime(index)">
          <view class="flex-start-center">
            <view class="cell-label">自提时间</view>
            <view class="cell-value">{{ item.date || '请选择' }} {{ item.startTime || '' }}{{ item.endTime ? '-' + item.endTime : '' }}</view>
          </view>
          <view><image class="arrow-horizontal" :src="require('@/assets/right11@2x.png')" mode="aspectFit"></image></view>
        </view>
      </view>
    ...
    <view class="app-card" v-else>
      <view class="flex-between cell" @click="addressS">
        <view class="flex-start-center">
          <view><image :src="require('@/assets/location60@2x.png')" mode="aspectFit" style="width: 68rpx;height: 68rpx;"></image></view>
          <view class="ml-20" style="color: #666;">
            <view>
              <text style="font-size: 32rpx;color: #1a1a1a;">{{ address.name }}</text>
              <text style="font-size: 24rpx;" class="ml-20">{{ address.phone }}</text>
            </view>
            <view style="font-size: 24rpx;">{{ address.detailAddress || '' }}</view>
          </view>
        </view>
        <view><image class="arrow-horizontal" :src="require('@/assets/right11@2x.png')" mode="aspectFit"></image></view>
      </view>
      ...
      <view class="flex-between cell">
        <view class="flex-start-center">
          ...
          <view class="cell-value">
            寄付
            <text class="ml-20" style="color: #0AB2C1;">￥{{ details.fare }}</text>
          </view>
        </view>
        <view><image class="arrow-horizontal" :src="require('@/assets/right11@2x.png')" mode="aspectFit"></image></view>
      </view>
    ...
    <view class="app-card">
      ...
      <view class="cell">{{ details.diseaseDesc }}</view>
      <view v-show="symptomShow">
        <view class="detailList flex-start-start"><image class="item" :src="FILE_URL(item)" v-for="(item, index) in details.rpPicIds" :key="index"></image></view>
        <view class="flex-start-center cell">
          <view class="cell-label">类型</view>
          <view class="cell-value">{{ switchBizType(details.serviceType) }}</view>
        </view>
        <view class="flex-start-center cell" @click="visitHistory()">
          <view class="cell-label">历史就诊记录</view>
          <view class="cell-value">{{ details.historyMedicalInfo.createTime||'无' }} {{ details.historyMedicalInfo.outDiagnose||'' }}</view>
        </view>
        <view class="flex-start-center cell">
          <view class="cell-label">诊疗形式</view>
          <view class="cell-value">{{ switchBizWay(details.serviceMode) }}</view>
        </view>
        <view>
          <view class="flex-between cell" v-show="details.serviceType == 'CARRYON_PRESC'">
            <view class="cell-label">续方处方</view>
            <view class="cell-value flex-center" @click="oldPrescriptionBoxShow = !oldPrescriptionBoxShow">
              <text style="margin-right: 12rpx;">{{ oldPrescriptionBoxShow ? '收起' : '展开' }}</text>
              <image class="arrow-horizontal" :src="require('@/assets/right11@2x.png')" mode="aspectFit"></image>
            </view>
          </view>
          <view v-show="oldPrescriptionBoxShow" class="prescription-box__item" v-for="(item, index) in details.oldRpList" :key="index">
          ...
          <view class="flex-between">
            <view class="medicine-name">{{ i + 1 }}.{{ rpDrug.name }} {{ rpDrug.spec }}</view>
            <view class="medicare">{{ rpDrug.medicare == 'ME_FEE' ? '自费' : swithMedicare(rpDrug.medicare) }}</view>
          </view>
          <view class="ml-20">
            <view class="use">
              {{ rpDrug.useWaysText || '' }} {{ rpDrug.useFrequencyText || '' }} {{ rpDrug.singleDose || '' }}{{ rpDrug.singleDoseUnitText + '/次' }} {{ rpDrug.treatment || ''
              }}{{ rpDrug.treatmentUnitText || '' }} ￥{{ rpDrug.price }}{{ '/' + rpDrug.totalUnitText }}x{{ rpDrug.total }}
            </view>
      ...
      <view class="text-center" @click="symptomShow = !symptomShow">
        <text style="color: #0AB2C1;">{{ symptomShow ? '收起' : '更多' }}服务订单信息</text>
        <image class="arrow-vertical" :class="{ reverse: symptomShow }" :src="require('@/assets/bottom-color@2x.png')" mode="aspectFit"></image>
      </view>
    </view>
    ...
    <view class="app-card prescription-box">
      <view class="flex-between cell">
        <view>处方</view>
        <view style="color: #999;" @click="prescriptionBoxShow = !prescriptionBoxShow">
          <text>{{ prescriptionBoxShow ? '收起' : '查看' }}</text>
          <image class="arrow-vertical" :src="require('@/assets/open@2x.png')" mode="aspectFit"></image>
        </view>
      </view>
      <view v-show="prescriptionBoxShow" class="prescription-box__item" v-for="(item, index) in details.rpDtoList" :key="index">
        <view class="flex-between">
          <view class="prescription-no">处方号：{{ item.id }}</view>
          <view class="prescription-price">￥{{ item.prescriptionPrice }}</view>
        </view>
        <view v-if="active == 'self'">自提点：{{item.rpDrugList[0].receivePharmacy}}</view>
        <view style="font-size: 24rpx;">数量:{{ item.number }} 医生：{{ item.doctorName }} {{ item.title }} {{ item.treatmentTime }}</view>
        <view class="">
          诊断：
          <text style="color: #333;">{{ item.diagnosis }}</text>
    ...
    <view class="app-card doctor-info">
      <view class="flex-start-start" style="padding: 20rpx 0;">
        <view class="avatar-wrapper"><image class="avatar" :src="details.docPic ? FILE_URL(details.docPic) : require('@/assets/user/user_d_head.png')" mode="scaleToFill"></image></view>
        <view class="ml-30">
          <view>
            <text class="doctor-info_doctor">{{ details.docName }}</text>
            <text>{{ details.docSex == '1' ? '男' : '女' }}</text>
          </view>
          <view class="">{{ details.docDept }} {{ details.docTitle }} {{ details.docHospital }}</view>
          <view class="text-overflow overflow-hidden" style="white-space: nowrap;width: 550rpx;">擅长：{{ details.docGoods }}</view>
        </view>
      </view>
    </view>
    ...
    <view class="app-card doctor-info" v-show="active == 'express'">
      <view class="flex-start-center cell">
        ...
        <view class="cell-value" style="color: #0AB2C1;">￥{{ prescriptionTotal }}</view>
      </view>
      <view class="flex-start-center cell">
        <view class="cell-label">运费</view>
        <view class="cell-value" style="color: #0AB2C1;">￥{{ details.fare }}</view>
      </view>
      <view class="flex-start-center cell">
        <view class="cell-label">合计</view>
        <view class="cell-value" style="color: #0AB2C1;">￥{{ getTotal() }}</view>
      </view>
    </view>

    <view class="flex-between mt-40">
      <view class="flex-center">
        <evan-checkBox style="margin-right: 10rpx;" v-model="confirmChecked" shape="square" primary-color="#0AB2C1" icon-size="12"></evan-checkBox>
        我同意
        <text style="color: #0AB2C1;" @click="agreementOpen()">{{ active == 'express' ? '《邮寄授权协议》' : '《自提授权协议》' }}</text>
      </view>
      <view style="color: #0AB2C1;" v-show="active == 'self'">合计￥{{ prescriptionTotal }}</view>
    </view>
    ...
    <uni-popup ref="visit_records">
      <view class="records-p">
        ...
        <view class="content">
          <view class="time" v-if="details.historyMedicalInfo.medicalType==='MZ'">
            <text>就诊记录</text>
            <text>{{ details.historyMedicalInfo.visitDate ||'无'}}</text>
          </view>
           <view class="time" v-else>
            <text>住院记录</text>
            <text>{{ (details.historyMedicalInfo.inDiagnose +'-'+ details.historyMedicalInfo.outDiagnose) ||'无'}}</text>
          </view>
          <text class="name">{{ details.historyMedicalInfo.patientName }}</text>
          <text class="doctor">{{ details.historyMedicalInfo.deptName }} {{ details.historyMedicalInfo.doctorName }} {{ details.historyMedicalInfo.title }}</text>
          <template v-if="details.historyMedicalInfo.medicalType==='MZ'">
            <text class="diagnosis">诊断：{{ details.historyMedicalInfo.diagnose }}</text>
          </template>
          <template v-else>
            <text class="diagnosis">入院诊断：{{ details.historyMedicalInfo.inDiagnose }}</text>
            <text class="diagnosis">出院诊断：{{ details.historyMedicalInfo.outDiagnose }}</text>
          </template>
        </view>
      </view>
    </uni-popup>
    <!--选择支付方式-->
    <popup-pay ref="pay" @success="paySuccess" @payTap="orderTap" :type="true" @cancel="payCancel"></popup-pay>

    <uni-popup ref="agreementPopup" type="center" :maskClick="false">
      <view class="type-popup">
        <text class="popup-title">{{ active == 'express' ? expressA.title : selfA.title }}</text>
        <scroll-view class="popup-content" scroll-y="true"><view v-html="active == 'express' ? expressA.content : selfA.content"></view></scroll-view>
...
import { userRpDetail, orderSubmit } from '../../common/request/userPrescription.js';
...
export default {
  ...
  methods: {
    ...
    selfTime(date, startTime, endTime) {
      this.details.storeDtoList[this.selectIndex].date = dayjs(date).format('YYYY-MM-DD');
      this.details.storeDtoList[this.selectIndex].startTime = this.timeFormat(startTime);
      this.details.storeDtoList[this.selectIndex].endTime = this.timeFormat(endTime);
      this.details.storeDtoList[this.selectIndex].orderId = this.details.rpDtoList[0].orderId;
      this.$forceUpdate();
    },
    timeFormat(time) {
 ...
 /*   async getHistory(id){
    ...
    getTotal() {
      if (this.active == 'self') {
        ...
        this.orderTotal = parseFloat(this.details.fare) + parseFloat(this.prescriptionTotal);
      }
