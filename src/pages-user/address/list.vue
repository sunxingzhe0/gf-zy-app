<template>
  <view class="doctorWrap">
    ...
    <view class="search flex-between box-shadow">
      <input type="text" style="width: 100%;" placeholderClass="searchtext" v-model="keywords" placeholder="按收货人/详细地址搜索" />
      <uni-icons type="search" class="searchIcon" size="20" @click="getaddressList"></uni-icons>
    </view>
    <view>
      <view class="list box-shadow" :class="select == 'orderSubmit' ? 'pointer':''" v-for="(item, index) in list" :key="index"
        ...
        <view class="info">
          ...
          <view class="des">{{ item.province }} {{ item.city }} {{ item.region }} {{ item.detailAddress }}</view>
          <view class="flex-between mt20">
            <view>
              <evan-checkbox v-model="item.defaultStatus" shape="square" @change="defauChange($event, item)"
                :primary-color="primaryColor">
                <text class="checkText">默认地址</text>
              </evan-checkbox>
            </view>
    ...
    <button class="submit_btn" type="primary" @click="add()">+新建收货地址</button>
  </view>
</template>
